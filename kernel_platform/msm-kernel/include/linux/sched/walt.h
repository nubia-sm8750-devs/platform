...
struct walt_task_struct {
	...
	u32				period_contrib_run;
        //nubia add begin
        /**
        * ÔÚÃ¿Ò»´Îwakeup_sucess·¢ÉúµÄÊ±ºò,last_pid¸úprev_last_pid
        * ¶¼»á½øÐÐ¼ÇÂ¼¡£¼ÇÂ¼¹æÔòÈçÏÂ¡£
        * ±äÁ¿Ê¹ÓÃµÄµØ·½ÔÚnbia_walt.cµÄwakeup_sucessº¯Êý 
        ...
        *            l_pid pl_pid
        *
        * meÖ¸µÄÊÇÄ¿±ê½ø³ÌµÄrender_tid
        * last_pid:»½ÐÑµ±Ç°rtidµÄwakerµÄÏß³Ìtid
        * prev_last_pid:»½ÐÑ[»½ÐÑrtidµÄwaker]µÄÏß³Ìtid
        * l_sexec_ts: last sum exec time
        * pl_sexec_ts: prev last sum exec time
        * ÕâÑù¾ÍÄÜÊµÏÖÍùÇ°×·ËÝµ÷ÓÃÁ´¡£
        **/
	...
	pid_t				pl_pid;//prev_last_pid
        ...
        * sched_clockÖ¸µÄÊÇwakeup·¢ÉúµÄÊ±¿Ì¡£
        *
        */
        u64 ringitem_build_expires;
        //affinity À¹½Ø»úÖÆ
        /*
        * is_app£¬ÊÇ·ñÊÇÈý·½Ó¦ÓÃ
        * persist_caller£¬¼ÇÂ¼ÊÇÄÄÌõusrspaceÏß³Ì·¢ÆðµÄaffinity³Ö¾Ã»¯ÒªÇó
        * affinity_persisted, ÊÇ·ñÊÇaffinityÊôÐÔ³Ö¾Ã»¯µÄÏß³Ì£»Èç¹ûÓÐ
        *     ±»usrspaceÇ¿ÖÆÒªaffinity³Ö¾Ã»¯£»¸ÃÖµÎªtrue¡£·ñÔòÎªfalse¡£
        * last£¬¼ÇÂ¼³ýÁË»úÖÆ½ÚµãÉèÖÃµÄaffinityÍâ£¬ÏµÍ³ÈÎÒâÆäËüÍ¨µÀ¶Ô
        *       task_structµÄcpu_maskµÄÖµ¡£°üÀ¨cpuset
        * req£¬¼ÇÂ¼usrspaceÒªÉèÖÃµÄaffinityÖµ£¬ÀíÂÛÉÏ³ýÁËÕâ¸öÖµ£¬È«ÏµÍ³°üÀ¨
        *     kernel¶ÔÕâÌõÏß³ÌµÄÐÞ¸ÄÖ»ÄÜÊÇÕâ¸öÖµ
        */
        bool is_app;
        bool mask_persisted;
        int persist_caller;

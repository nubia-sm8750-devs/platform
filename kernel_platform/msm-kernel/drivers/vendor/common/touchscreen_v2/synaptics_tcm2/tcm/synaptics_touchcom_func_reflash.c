...
/*
 *  Confirm that the bootloader firmware is activate.
 *
 * param
 *    [ in] tcm_dev:         the TouchComm device handle
...
/*
 *  Wrap up a TouchComm message to communicate with bootloader firmware.
 *
 * param
 *    [ in] tcm_dev:        the TouchComm device handle
...
/*
 *  Assemble a TouchComm bootloader command to read data from the flash.
...
/*
 *  Read the data of lockdown config in the flash memory.
 *
 * param
 *    [ in] tcm_dev:      the TouchComm device handle
 *    [ in] reflash_data: data blob for reflash
...
/*
 *  Read the data of customer serialization area in the flash memory.
 *
 * param
 *    [ in] tcm_dev:      the TouchComm device handle
 *    [ in] reflash_data: data blob for reflash
	...
	unsigned int resp_reading)
{
	...
	retval = syna_tcm_buf_alloc(rd_data, boot_config.data_length);
	if (retval < 0) {
		LOGE("Fail to allocate memory for rd_data buffer\n");
		goto exit;
	}
	...
	for (i = 0; i < BOOT_CONFIG_SLOTS; i++) {
		if ((boot_config.buf[offset + 1] >> 7) == 1) {
			if ((boot_config.buf[offset - BOOT_CONFIG_SLOT_SIZE + 1] >> 7) == 0)
				break;
		}
	...
	retval = syna_pal_mem_cpy(rd_data->buf, rd_data->buf_size,
		&boot_config.buf[0], boot_config.buf_size, offset);
	if (retval < 0) {
		LOGE("Fail to copy data to rd_data buffer\n");
		goto exit;
	}

	rd_data->data_length = offset;
...
/*
 *  Request to read out data from the flash memory.
 *
 * param
 *    [ in] tcm_dev:       the TouchComm device handle
 *    [ in] area:          flash area to read
 *                         set AREA_NONE to assign the custom flash address to read
 *    [ in] address:       custom flash address to read
 *                         that is used only when area is AREA_NONE
 *    [ in] length:        custom size to read
 *                         that is used only when area is AREA_NONE
...
int syna_tcm_read_flash_area(struct tcm_dev *tcm_dev, enum flash_area area,
	unsigned int address, unsigned int length, struct tcm_buffer *rd_data,
	unsigned int resp_reading)
{
	int retval = 0;
...
/*
 *  Assemble a TouchComm bootloader command to write data to the flash.
 *
...
/*
 *  Assemble a TouchComm bootloader command to do optimized data writing.
 *
 *  If the length of the data to write is not an integer multiple of words,
 *  the trailing byte will be discarded.  If the length of the data to write
 *  is not an integer number of write blocks, it will be zero-padded to the
...
/*
 *  Write data to the target data partition in the flash memory.
 *
 * param
 *    [ in] tcm_dev:       the TouchComm device handle
 *    [ in] reflash_data:  data blob for reflash
...
/*
 *  Assemble a TouchComm bootloader command to erase the requested pages
 *  of the flash memory.
...
/*
 *  Implement the sequence to update the target area in the flash memory.
 *
 * param
 *    [ in] tcm_dev:               the TouchComm device handle
 *    [ in] reflash_data:          data blob for reflash
...
/*
 *  Implement the sequence to update the lockdown config from the image file to device.
 *
 * param
 *    [ in] tcm_dev:               the TouchComm device handle
 *    [ in] image:                 parsed data from image file
 *    [ in] flash_access_delay_us: delay time to wait for the completion of flash write
 *                                   for polling,     set a positive value;
 *                                   for ATTN-driven, use '0' or 'RESP_IN_ATTN'
 *    [ in] fw_switch_delay_ms:     set up the ms delay time to wait for the completion of firmware switching
 *                                    for polling,     set a positive value;
...
int syna_tcm_update_lockdown_config(struct tcm_dev *tcm_dev, struct image_info *image,
	unsigned int flash_access_delay_us, unsigned int fw_switch_delay_ms)
{
	int retval;
	struct tcm_reflash_data_blob reflash_data;
	struct tcm_buffer boot_config;
	...
	if (flash_access_delay_us == CMD_RESPONSE_IN_ATTN)
		resp_handling = CMD_RESPONSE_IN_ATTN;
	else
		resp_handling = tcm_dev->msg_data.command_polling_time;
	...
	for (i = 0; i < (total_bytes_to_update / BOOT_CONFIG_SLOT_SIZE); i++) {
		...
		retval = syna_tcm_write_flash(tcm_dev, &reflash_data,
			...
			BOOT_CONFIG_SLOT_SIZE, resp_handling);
		if (retval < 0)
			goto exit;
	}
...
/*
 *  Implement the sequence to write the data to the custom serialization area
 *
 * param
 *    [ in] tcm_dev:               the TouchComm device handle
...
int syna_tcm_update_cs_config(struct tcm_dev *tcm_dev, unsigned char *cs_data,
	unsigned int cs_data_size, unsigned int cs_offset, unsigned int flash_access_delay_us,
	unsigned int fw_switch_delay_ms)
{
	int retval;
	struct tcm_reflash_data_blob reflash_data;
	struct tcm_buffer boot_config;
	...
	retval = syna_tcm_write_flash(tcm_dev, &reflash_data, addr, &boot_config.buf[offset],
			reflash_data.total_bytes_to_update, resp_handling);
	if (retval < 0)
		goto exit;

	...
	unsigned int wr_size, unsigned int rd_size)
{
	...
	max_size = syna_pal_le2_to_uint(id_info->max_read_size);
	...
	rd_size = (rd_size > max_size) ? max_size : rd_size;
	if (rd_size != current_read_size) {
		...
		if (tcm_dev->write_message) {
			retval = tcm_dev->write_message(tcm_dev,
				CMD_TCM2_SET_MAX_READ_LENGTH, data, sizeof(data), NULL, 0);
			if (retval < 0) {
				LOGE("Fail to set max read size\n");
				return retval;
			}
			rd_size = current_read_size;
		}
	}
...
static int syna_tcm_v2_check_max_rw_size(struct tcm_dev *tcm_dev)
{
	...
	rd_size = MIN(rd_size, current_read_size);
	...
	if (wr_size != tcm_dev->max_wr_size)
		wr_size = (tcm_dev->max_wr_size == 0) ? wr_size : MIN(wr_size, tcm_dev->max_wr_size);

	/* check the max read size between the identify report and the platform's settings */
	if (rd_size != tcm_dev->max_rd_size)
		rd_size = (tcm_dev->max_rd_size == 0) ? rd_size : MIN(rd_size, tcm_dev->max_rd_size);

	if (tcm_dev->hw->alignment_enabled) {
		wr_size = syna_pal_int_alignment(wr_size, tcm_dev->hw->alignment_base, false);
		rd_size = syna_pal_int_alignment(rd_size, tcm_dev->hw->alignment_base, false);
	}
	...
	bool resend, unsigned char *buf, unsigned int buf_size, unsigned int *packet_size)
{
	...
	if (payload_size != header_length)
		LOGD("Length in header:%d, size to write:%d\n", header_length, payload_size);
...
static int syna_tcm_v2_read(struct tcm_dev *tcm_dev, unsigned int rd_length, bool ignore_corrupt_read)
{
	...
	int xfer_len = (int)sizeof(struct tcm_v2_message_header);
	int retry = 0;
	...
	do {
		...
		LOGD("Data %02x %02x %02x %02x (seq:%d, crc6:%02X)\n",
			header->data[0], header->data[1], header->data[2], header->data[3],
			((header->byte3 & 0x40) >> 6), (header->byte3 & 0x3f));
		...
		if (retval < 0) {
			...
			LOGW("Unknown packet, do retry %d\n", ++retry);
			syna_pal_sleep_ms(WR_RD_RETRY_DELAY);
		}
	} while ((retval < 0) && (retry < WR_RD_RETRY_TIMES));
	...
	bool has_first_chunk)
{
	...
	for (iterations = 1; iterations < chunks; iterations++) {
		...
		resend = false;
		do {
			retval = syna_tcm_v2_write(tcm_dev, CMD_TCM2_ACK, NULL, 0, 0, resend);
			if (retval < 0) {
	...
	do {
		...
		do {
			...
			if (retval < 0) {
				...
				if ((retval != -PACKET_CORRUPTED) && (retval != -PACKET_RETRY_REQUEST))
					LOGD("Attempt to read in the response again, retry %d\n", timeout);
			}
	...
	unsigned char *status_report_code)
{
	...
	/* request a command */
	retval = syna_tcm_v2_process_command(tcm_dev, CMD_TCM2_GET_REPORT, NULL, 0, 0);
	if (retval < 0) {
	...
	unsigned int resp_reading)
{
	...
	for (iterations = 0; iterations < chunks; iterations++) {
		...
		retval = syna_tcm_v2_process_command(tcm_dev,
			...
			&payload[offset], xfer_length, payload_length);
		if (retval < 0) {

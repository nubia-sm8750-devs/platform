...
extern int syna_ts_check_dt(struct device *dev);
#endif

/*ZTE for add CS_TO_CLK_DELAY*/
struct spi_geni_qcom_ctrl_data {
	u32 field1;
	u32 field2;
};
/*max 1023;1022:frequency1M cs_to_clk 270us;frequency10M cs_to_clk 26us*/
#define CS_TO_CLK_DELAY 1022   //900 frequency10M cs_to_clk 14.8us
		...
		struct device *dev)
{
	...
	LOGI("Load from dt: command timeout(%d) turnaround time(%d %d) retry time(%d)\n",
		product->default_cmd_timeout_ms, product->default_cmd_turnaround_us[0],
		product->default_cmd_turnaround_us[1], product->default_cmd_retry_ms);
...
static int syna_spi_request_bus_resources(struct syna_hw_interface *hw_if)
{
	...
	struct spi_geni_qcom_ctrl_data *ctrl_data;
...
/* Started by AICoder, pid:06a30b5fccic54f1452d0a79f0cd63012d979f5e */
	ctrl_data = kzalloc(sizeof(struct spi_geni_qcom_ctrl_data), GFP_KERNEL);
	if (!ctrl_data) {
    		LOGE("Fail to allocate ctrl_data\n");
    		return -ENOMEM;
	}
	ctrl_data->field1 = CS_TO_CLK_DELAY;
	spi->controller_data = ctrl_data;
/* Ended by AICoder, pid:06a30b5fccic54f1452d0a79f0cd63012d979f5e */
...
static int syna_spi_power_on(struct syna_hw_interface *hw_if, bool on)
{
	...
	if (on) {
		...
		if (pwr->vio.control > 0) {
			/* power off VIO */
			if (pwr->vio.control == PSU_REGULATOR) {
				retval = regulator_disable((struct regulator *)pwr->vio.regulator_dev);
				LOGI("vio disable successs:%d", retval);
			} else {
				if (pwr->vio.gpio > 0)
					gpio_set_value(pwr->vio.gpio, !pwr->power_on_state);
			}
		...
		if (pwr->vdd.control > 0) {
			/* power off VDD */
			if (pwr->vdd.control == PSU_REGULATOR) {
				retval = regulator_disable((struct regulator *)pwr->vdd.regulator_dev);
				LOGI("vdd disable successs:%d", retval);
			} else {
				if (pwr->vdd.gpio > 0)
					gpio_set_value(pwr->vdd.gpio, !pwr->power_on_state);
			}
		...
		gpio_direction_output(rst->reset_gpio, 0);
		usleep_range(10000, 11000);
		LOGI("reset down successs");
	}
	...
	unsigned int wr_len, unsigned char *rd_data, unsigned int rd_len, unsigned int turnaround_bytes)
{
...
#ifdef DEBUG_BUS_DATA_TRACE
	unsigned char *dbg_wr_str;
	unsigned char *dbg_rd_str;
	int dbg_wr_len = (wr_len > DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG) ? DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG : wr_len;
	int dbg_rd_len = (rd_len > DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG) ? DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG : rd_len;
	...
	if (dbg_wr_str) {
		...
		if (wr_len > DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG)
	...
	if (dbg_rd_str) {
		...
		if (rd_len > DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG)
			strlcat(dbg_rd_str, "...", dbg_rd_len * 3 + 3);
	}
	LOGD("WR-THEN-RD size:%d WR:%d [%s] -- RD:%d [%s]\n", total_length, wr_len, dbg_wr_str, rd_len, dbg_rd_str);
	...
	unsigned int rd_len)
{
	...
	int dbg_len = (rd_len > DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG) ? DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG : rd_len;
...
#ifdef DEBUG_BUS_DATA_TRACE
	unsigned char *dbg_str;
	int dbg_len = (wr_len > DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG) ? DEBUG_MAX_BUS_DATA_LENGTH_TO_DEBUG : wr_len;
...
#ifdef TOUCHCOMM_VERSION_2
		.ops_write_then_read_data = syna_spi_write_then_read,
#endif
...
#ifdef CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER
	retval = syna_ts_check_dt(&spi->dev);
#endif

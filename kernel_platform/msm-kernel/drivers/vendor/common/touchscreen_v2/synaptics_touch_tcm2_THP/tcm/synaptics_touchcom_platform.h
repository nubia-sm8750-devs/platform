...
struct tcm_hw_platform {
	...
	/* flag recording the bus interface, that enumerates as the bus_connection */
	unsigned char type;
	/* capability of I/O chunk size */
	unsigned int rd_chunk_size;
	unsigned int wr_chunk_size;

	/* indicates whether interrupt (ATTN) support is available.
	...
	/* abstraction to read in data from bus
	 *
	 * param

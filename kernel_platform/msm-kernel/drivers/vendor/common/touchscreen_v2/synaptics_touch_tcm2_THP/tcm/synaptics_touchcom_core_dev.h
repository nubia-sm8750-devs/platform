...
/*
 * This file serves as the primary header for the TouchComm protocol, providing the main
 * context structure and definitions required for communication and data handling.
 */
...
#define SYNA_TCM_CORE_LIB_VERSION		0x020E
 ...
 * Touchcomm Protocols
 *
 *   PROTOCOL_DETECT_VERSION_1     :  Indicates use of protocol version 1
 *   PROTOCOL_DETECT_VERSION_2     :  Indicates use of protocol version 2
 *   PROTOCOL_BYPASS_STARTUP_PACKET:  Requests bypass of the startup packet
 */
...
/*
 * Default Timing Parameters for Reflash Operations
 *
 *    DEFAULT_FLASH_ERASE_DELAY_US : Delay for flash erase per page (in microseconds)
 *    DEFAULT_FLASH_WRITE_DELAY_US : Delay for flash write per block (in microseconds)
 *    DEFAULT_FLASH_READ_DELAY_US  : Delay for flash read per word (in microseconds)
 */
...
enum tcm_command {
	...
	CMD_SET_GAME_PARTITION_CONFIG = 0x78,
...
/* Callback to the Handling of Post Reset
 *
...
/* Callback to the Handling of Message
 *
 * Definitions of callback function
 * param
...
/* Callback to Parse the Custom Touch Entity
 *
 * Definitions of callback function
 * param
...
struct tcm_dev {
	...
	/* abstraction to read in a TouchComm message from device
	 *
	 * param
	...
	/* callback to dispatch the received report */
	struct tcm_message_callback cb_report_dispatcher[MAX_REPORT_TYPES];
	/* callback to duplicate the data to the caller */
...
#endif
	/* callback to handle the operation for the post reset */
 ...
 *  Functions are implemented in synaptics_tcm2_core_v1.c.
 *
 * param
 *    [ in] tcm_dev:  the TouchComm device handle
 ...
 *  Functions are implemented in synaptics_tcm2_core_v2.c.
 *
 * param
 *    [ in] tcm_dev:  the TouchComm device handle
 *    [ in] bypass:   flag to bypass the detection

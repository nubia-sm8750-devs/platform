...
/*
 *  Helper to Set the Timing Parameters for Command Processing
 *
 * param
	...
	unsigned int setting, unsigned int type)
{
	...
	if ((type & TIMINGS_CMD_RETRY) == TIMINGS_CMD_RETRY) {
		if (product) {
			time = product->cmd_retry_ms;
			if (time != 0)
				tcm_msg->retry_time = product->cmd_retry_ms;

			LOGD("Set timing: Command retry time(%d)\n", tcm_msg->retry_time);
		}
	}
...
/*add for game partition*/
int syna_tcm_set_game_partition_config(struct tcm_dev *tcm_dev, unsigned char id,
	int len, unsigned char *value, unsigned int resp_reading)
{
	...
	unsigned char*out = kmalloc(len+1, GFP_KERNEL);
...
#ifndef SW_VALIDATION
	if (IS_NOT_APP_FW_MODE(tcm_dev->dev_mode)) {
		LOGN("Device is not in application fw mode, mode: %x\n",
			tcm_dev->dev_mode);
		return -ERR_INVAL;
	}
#endif
	...
	if (resp_reading == CMD_RESPONSE_IN_ATTN) {
		if (!tcm_dev->hw->support_attn) {
			resp_reading = tcm_dev->msg_data.command_polling_time;
			LOGE("No support of IRQ control, use polling mode instead\n");
		}
	}
    ...
    if (log_buf) {
        log_index += snprintf(log_buf, log_size, "out values: ");
    }
	...
	for(int i = 1; i < len + 1; i++) {
		out[i] = value[i-1];
		if (log_buf && (log_index < log_size - 8)) {
			log_index += snprintf(log_buf + log_index,
							log_size - log_index,
							"%02x ", out[i]);
		}
	}
	...
	if (retval < 0) {
		LOGE("Fail to send command 0x%02x to set %d to field 0x%x\n",
			CMD_SET_GAME_PARTITION_CONFIG, len + 1, (unsigned char)id);
		goto exit;
	}

...
static void syna_ts_panel_notifier_callback(enum panel_event_notifier_tag tag,
...
extern void syna_tpd_register_fw_class(struct syna_tcm *tcm);
...
static void syna_dev_report_input_events(struct syna_tcm *tcm)
{
	...
	for (idx = 0; idx < max_objects; idx++) {
		...
		case LIFT:
			...
			if (cdev->zte_tp_algo) {
				tpd_touch_release(input_dev, idx);
			} else {
#ifdef TYPE_B_PROTOCOL
				input_mt_slot(input_dev, idx);
...
#ifdef GTP_REPORT_BY_ZTE_ALGO
			if (cdev->zte_tp_algo) {
				LOGD("report: id[%d], x %d, y %d, w %d", idx, x, y, MAX(wx, wy));
				tpd_touch_press(input_dev, x, y, idx, MAX(wx, wy), 0);
			} else {
#ifdef TYPE_B_PROTOCOL
				input_mt_slot(input_dev, idx);
				input_mt_report_slot_state(input_dev,
				...
				input_mt_sync(input_dev);
#endif
				LOGI("Finger %d: x = %d, y = %d\n", idx, x, y);
			}
#else/*GTP_REPORT_BY_ZTE_ALGO*/
#ifdef TYPE_B_PROTOCOL
			input_mt_slot(input_dev, idx);
			input_mt_report_slot_state(input_dev,
					MT_TOOL_FINGER, 1);
...
static irqreturn_t syna_dev_isr(int irq, void *data)
{
	...
	if (tcm->pwr_state != PWR_ON) {
		...
		while (count) {
			if (pm_resume_flag) {
				LOGI("pm resume , begin spi , time consuming is %d ms", (100 - count) * 5);
				break;
			}
...
#define SYNA_SPI_MAX_FREQUENCY 10000000
static int syna_change_spi_frequency(struct syna_hw_interface *hw_if) {
	int retval;
	...
	spi->max_speed_hz = SYNA_SPI_MAX_FREQUENCY;
	...
	LOGI("spi frequency set success\n");
	return 0;
}
...
static void syna_dev_reflash_startup_work(struct work_struct *work)
{
	...
	LOGI("fw update success\n");

	syna_tpd_register_fw_class(tcm);
	retval = syna_change_spi_frequency(hw_if);
	if (retval < 0) {
		LOGE("Fail to change spi frequency\n");
	}
exit:
...
int syna_dev_set_tp_report_rate(struct syna_tcm *tcm, int en,
	unsigned int resp_handling)
{
	static int current_report_mode = tp_freq_120Hz;
	int retval = 0;
	...
	switch (en) {
		...
		case tp_freq_960Hz:
			...
			break;
		default:
			LOGE("%s: enable not support", __func__);
			return 0;
	}
...
int syna_dev_set_sensibility_level(struct syna_tcm *tcm, int en,
	unsigned int resp_handling)
{
	int retval = 0;
	...
	switch (en) {
		case sensibility_level_0:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | tcm->ztec.follow_hand_level << 4 | 0),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in sensibility_level_0", __func__);
			break;
		case sensibility_level_1:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | tcm->ztec.follow_hand_level << 4 | 1),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in sensibility_level_1", __func__);
			break;
		case sensibility_level_2:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | tcm->ztec.follow_hand_level << 4 | 2),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in sensibility_level_2", __func__);
			break;
		case sensibility_level_3:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | tcm->ztec.follow_hand_level << 4 | 3),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in sensibility_level_3", __func__);
			break;
		case sensibility_level_4:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | tcm->ztec.follow_hand_level << 4 | 4),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in sensibility_level_4", __func__);
			break;
		default:
			LOGE("%s: enable not support", __func__);
			return 0;
...
int syna_dev_set_follow_hand_level(struct syna_tcm *tcm, int en,
	unsigned int resp_handling)
{
	int retval = 0;
	...
	switch (en) {
		case follow_hand_level_0:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | 0 << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in follow_hand_level_0", __func__);
			break;
		case follow_hand_level_1:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | 1 << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in follow_hand_level_1", __func__);
			break;
		case follow_hand_level_2:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | 2<< 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in follow_hand_level_2", __func__);
			break;
		case follow_hand_level_3:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | 3 << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in follow_hand_level_3", __func__);
			break;
		case follow_hand_level_4:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(tcm->ztec.stability_level << 8 | 4 << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in follow_hand_level_4", __func__);
			break;
		default:
			LOGE("%s: enable not support", __func__);
			return 0;
...
int syna_dev_set_stability_level(struct syna_tcm *tcm, int en,
	unsigned int resp_handling)
{
	int retval = 0;
	...
	switch (en) {
		case stability_level_0:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(0 << 8 | tcm->ztec.follow_hand_level << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in stability_level_0", __func__);
			break;
		case stability_level_1:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(1 << 8 | tcm->ztec.follow_hand_level << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in stability_level_1", __func__);
			break;
		case stability_level_2:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(2 << 8 | tcm->ztec.follow_hand_level<< 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in stability_level_2", __func__);
			break;
		case stability_level_3:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(3 << 8 | tcm->ztec.follow_hand_level << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in stability_level_3", __func__);
			break;
		case stability_level_4:
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
					0xFA,
					(4 << 8 | tcm->ztec.follow_hand_level << 4 | tcm->ztec.sensibility_level),
					resp_handling);
			if (retval < 0) {
				return retval;
			}
			LOGI("%s success in stability_level_4", __func__);
			break;
		default:
			LOGE("%s: enable not support", __func__);
			return 0;
...
/* display_rotation */
int syna_dev_set_display_rotation(struct syna_tcm *tcm, int mrotation,
	unsigned int resp_handling)
{
	int retval = 0;
	int level = tcm->ztec.rotation_limit_level;

	LOGI("%s: set display_rotation state %d\n", __func__, mrotation);
	...
	LOGI("%s success set edge suppression", __func__);
	...
	LOGI("%s success set corer suppression level is %d", __func__, level);
	...
	LOGI("%s success set mrotation is %d", __func__, mrotation);
...
/* play_game */
int syna_dev_set_play_game(struct syna_tcm *tcm, int enable,
	unsigned int resp_handling)
{
	int retval = 0;
...
{
	pm_resume_flag = 1;
	LOGI("%s: pm_resume_flag:%d", __func__, pm_resume_flag);
...
/* Started by AICoder, pid:29763q9e05qc64914f010a0ed1ac86533383f4e9 */
static int syna_panel_resume(struct device *dev)
{
    int retval;
    struct syna_tcm *tcm = dev_get_drvdata(dev);
    struct tcm_dev *tcm_dev;
	...
	if (IS_NOT_APP_FW_MODE(tcm_dev->dev_mode)) {
		LOGW("Device is not in the application mode after resume\n");
		goto exit;
	}//SUNYU
...
exit:
	...
	retval = syna_dev_set_display_rotation(tcm, tcm->ztec.display_rotation, resp_handling);

	if (tcm->ztec.is_play_game) {
		retval = syna_dev_set_play_game(tcm, 1, resp_handling);
		if (retval)
  			LOGE("set game mode failed!");
		retval = syna_dev_set_tp_report_rate(tcm, tcm->ztec.tp_report_rate, resp_handling);
		retval = syna_dev_set_sensibility_level(tcm, tcm->ztec.sensibility_level, resp_handling);
		retval = syna_dev_set_follow_hand_level(tcm, tcm->ztec.follow_hand_level, resp_handling);
		retval = syna_dev_set_stability_level(tcm, tcm->ztec.stability_level, resp_handling);
	}
    /* enable irq */
 ...
 *    0 or positive value in case of success, a negative value otherwise.
 */
static int syna_panel_suspend(struct device *dev)
{
    struct syna_tcm *tcm = dev_get_drvdata(dev);
    struct syna_hw_interface *hw_if = tcm->hw_if;
    ...
    tcm->lpwg_enabled = tcm->ztec.is_wakeup_gesture;
    LOGI("lpwg_enabled is %d", tcm->lpwg_enabled);
...
/* Ended by AICoder, pid:29763q9e05qc64914f010a0ed1ac86533383f4e9 */
...
int syna_ts_check_dt(struct device *dev)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (PTR_ERR(panel) == -ENODEV) {
			LOGI("%s: no device!", __func__);
			retval = -ENODEV;
		} else if (PTR_ERR(panel) == -EPROBE_DEFER) {
			LOGI("%s: device has not been probed yet", __func__);
			retval = -EPROBE_DEFER;
		}
...
static void syna_ts_register_for_panel_events(struct syna_tcm *tcm)
{
	...
	cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
			...
			&syna_ts_panel_notifier_callback, tcm);
	if (!cookie) {
		LOGE("Failed to register for panel events\n");
		return ;
	}

	LOGI("registered for panel notifications panel: 0x%p\n",
			active_panel);
...
static int tpd_goodix_ts_resume(void *_tcm)
{
	struct syna_tcm *tcm = (struct syna_tcm *)_tcm;

	return syna_panel_resume(&tcm->pdev->dev);
}

static int tpd_goodix_ts_suspend(void *_tcm)
{
	struct syna_tcm *tcm = (struct syna_tcm *)_tcm;

	return syna_panel_suspend(&tcm->pdev->dev);
}

static void tpd_resume_suspend_register(struct syna_tcm *tcm)
{
	LOGI("tpd_resume_suspend_register IN");
	tpd_cdev->tp_data = tcm;
...
static int syna_dev_probe(struct platform_device *pdev)
{
	...
	timings.cmd_retry_ms = hw_if->product.default_cmd_retry_ms;
...
/*#ifdef ENABLE_WAKEUP_GESTURE
...
#ifdef CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER
	syna_ts_register_for_panel_events(tcm);
#endif

	tpd_resume_suspend_register(tcm);
	...
	tcm->ztec.sensibility_level = 2;
	tcm->ztec.follow_hand_level = 2;
	tcm->ztec.stability_level = 2;
	tcm->ztec.rotation_limit_level = 2;
	LOGI("set default game mode success");
...
#ifdef CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER
	if (active_panel && tcm->notifier_cookie)

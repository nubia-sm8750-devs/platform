...
static ssize_t syna_sysfs_get_delta_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	...
	retval = syna_tcm_enable_report(tcm_dev, report, true, 10);
	if (retval < 0) {
		return scnprintf(buf, PAGE_SIZE, "Fail to enable report %x\n", report);;
	}

	syna_tcm_enable_irq(tcm_dev, false);

	syna_tcm_buf_init(&buffer);

	count = 0;

	while ((count < 2) && (retry < 5)) {
		retval = syna_tcm_wait_for_report(tcm_dev, report, &buffer, 100, 1000);
		if ((retval < 0) || (buffer.data_length <= 0)) {
			retry += 1;
			continue;
		}
		LOGI("Delta Frame %d ...\n", (count + 1));
		s16_data = (short *)buffer.buf;
		for (row = 0; row < tcm_dev->rows; row++) {
			str = syna_pal_mem_alloc(tcm_dev->cols * 6, sizeof(unsigned char));
			for (col = 0; col < tcm_dev->cols; col++) {
				unsigned char strbuff[6];

				syna_pal_mem_set(strbuff, 0x00, 6);
				snprintf(strbuff, 6, "%4d ", *s16_data);
				strlcat(str, strbuff, tcm_dev->cols * 6);
				s16_data++;
			}
			LOGI("%s\n", str);
			syna_pal_mem_free(str);
		}

		count += 1;
	}

	syna_tcm_enable_report(tcm->tcm_dev, report, false, 10);
...
static struct kobj_attribute kobj_attr_get_delta =
	__ATTR(get_delta, 0444, syna_sysfs_get_delta_show, NULL);
		...
		struct platform_device *pdev)
{
	...
	retval = sysfs_create_group(tcm->sysfs_dbg_dir, &attr_debug_group);
	if (retval < 0) {
		LOGE("Fail to create sysfs debug group\n");

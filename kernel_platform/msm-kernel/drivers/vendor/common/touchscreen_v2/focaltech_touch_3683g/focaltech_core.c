...
extern unsigned long start_exit_fold_timer;
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    FTS_DEBUG("fts_get_ic_information in");
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    struct ts_event *events = ts_data->events;
    unsigned int end_exit_fold_timer = jiffies_to_msecs(jiffies - start_exit_fold_timer);
    ...
    if (end_exit_fold_timer < 800) {
        FTS_INFO("Just exit fold, no need to judge touch irq");
        return 0;
    }
    ...
    case TOUCH_DEFAULT:
        ...
        for (i = 0; i < max_touch_num; i++) {
            ...
            if (is_recovery == ZTE_IN_RECOVERY) {
		        events[i].x = events[i].x / ZTE_FTS_SR;
		        events[i].y = events[i].y / ZTE_FTS_SR;
	        }
...
static int focal_ts_power_init(struct fts_ts_data *ts_data)
{
	...
	if (strlen(ts_bdata->iovdd_name)) {
		ts_data->vdd = devm_regulator_get(dev,
				 ts_bdata->iovdd_name);
		if (IS_ERR_OR_NULL(ts_data->vdd)) {
			ret = PTR_ERR(ts_data->vdd);
			FTS_ERROR("Failed to get regulator vdd:%d", ret);
			ts_data->vdd = NULL;
		}
		ret = regulator_set_voltage(ts_data->vdd, 1800000, 1800000);
		if (ret < 0) {
			FTS_ERROR("set vdd voltage failed");
			return ret;
		}
	} else {
		FTS_INFO("vdd name is NULL");
	}

	ts_data->vdd_enable_gpio = ts_bdata->vdd_enable_gpio;
	ret = devm_gpio_request_one(dev, ts_data->vdd_enable_gpio,
						GPIOF_OUT_INIT_LOW, "TP_VDD_GPIO");
	if (ret) {
		FTS_ERROR("failed to request vdd_enable_gpio");
		return ret;
	}
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			ret = gpio_direction_output(ts_data->vdd_enable_gpio, 1);
			if (ret) {
		...
		if (!ts_data->power_disabled) {
			...
			ret = gpio_direction_output(ts_data->vdd_enable_gpio, 0);
			if (ret) {
                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    if (is_recovery == ZTE_IN_RECOVERY) {
		pdata->x_min = pdata->x_min / ZTE_FTS_SR;
		pdata->y_min = pdata->y_min / ZTE_FTS_SR;
		pdata->x_max = pdata->x_max / ZTE_FTS_SR;
        pdata->y_max = pdata->y_max / ZTE_FTS_SR;
	}
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (!ret) {
		FTS_INFO("iovdd_name name from dt: %s", name_tmp);
	...
	ret = of_property_read_string(np, "focal,avdd-name", &name_tmp);
	if (!ret) {
		FTS_INFO("avdd_name name from dt: %s", name_tmp);
	...
	if (strlen(pdata->avdd_name)) {
		...
		if (IS_ERR_OR_NULL(pdata->avdd)) {
			...
			FTS_ERROR("Failed to get regulator avdd:%d", ret);
			pdata->avdd = NULL;
		}
		if (ret < 0) {
			FTS_ERROR("set avdd voltage failed");
			return ret;
		}
	} else {
    ...
    ret = gpio_direction_output(pdata->avdd_enable_gpio, 1);*/
    ret = regulator_enable(pdata->avdd);
	if (ret) {
		FTS_ERROR("enable avdd regulator failed,ret=%d", ret);
		return ret;
	}
    FTS_DEBUG("avdd regulator enable !");
    ...
    FTS_DEBUG("iovdd regulator enable !");
    usleep_range(1000, 1100);
...
int fts_ts_suspend(struct device *dev)
{
    ...
    usleep_range(3000, 3100);
    FTS_INFO("wait 3ms for juduge fold state");
    if (ts_data->ztec.is_smart_cover || !ts_data->gesture_support) {
        FTS_INFO("make TP enter into sleep mode");
        FTS_INFO("is_single_tap:%d, is_wakeup_gesture%d", ts_data->ztec.is_single_tap, ts_data->ztec.is_wakeup_gesture);
        usleep_range(3000, 3100);
        fts_irq_disable();
        ...
        FTS_INFO("fts_gesture suspend, status = %d", status);
    }
...
int fts_ts_resume(struct device *dev)
{
    ...
    fts_irq_enable();
    if(ts_data->ztec.tp_report_rate == 0) {
        fts_write_reg(FTS_REG_REPORT_RATE, 0x0C);
        FTS_INFO("resume enter_120_HZ success\n");
    }

    if(ts_data->ztec.is_play_game) {
		fts_write_reg(FTS_REG_REPORT_RATE, 0x30);
		fts_write_reg(FTS_REG_GAME_MODE, 1);
		FTS_INFO("resume enter_play_game success\n");
    }

    if (ts_data->ztec.is_single_fp) {
		fts_write_reg(FTS_REG_FOD_EN, FTS_REG_FOD_VALUE);
		FTS_INFO("FOD resume wirte 0x2 success");
	}

    if (ts_data->ztec.is_smart_cover) {
		fts_write_reg(FTS_REG_COVER_MODE_EN, 1);
		FTS_INFO("smart_cover resume wirte 0x2 success");
	}
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("is_recovery = %d", is_recovery);
...
*/
    ...
    if (ret) {
        ...
        FTS_INFO("init fw upgrade success");
    }

...
unsigned long start_exit_fold_timer;
...
static int fts_get_wakegesture(struct ztp_device *cdev)
{
	struct fts_ts_data *ts_data = (struct fts_ts_data *)cdev->private;
...
static int fts_enable_wakegesture(struct ztp_device *cdev, int enable)
{
	struct fts_ts_data *ts_data = (struct fts_ts_data *)cdev->private;
	...
	if (ts_data->suspended) {
		...
		switch (tp_report_rate_level) {
			case tp_freq_120Hz:
				...
				FTS_INFO("success in tp_freq_120Hz");
				if (ret < 0) {
			...
			case tp_freq_240Hz:
				...
				FTS_INFO("success in tp_freq_240Hz");
				if (ret < 0) {
					FTS_ERROR("%s write report_rate fail", __func__);
				}
				break;
			case tp_freq_360Hz:
				...
				FTS_INFO("success in tp_freq_360Hz");
				if (ret < 0) {
					FTS_ERROR("%s write report_rate fail", __func__);
				}
				break;
			case tp_freq_480Hz:
				...
				FTS_INFO("success in tp_freq_480Hz");
				if (ret < 0) {
					FTS_ERROR("%s write report_rate fail", __func__);
				}
				break;
...
static int tpd_set_smart_cover(struct ztp_device *cdev, int enable)
{
	...
	ts_data->ztec.is_smart_cover = enable;
	if (!enable) {
		start_exit_fold_timer = jiffies;
	}
	if (enable && ts_data->suspended) {
		FTS_INFO("enter_smart_cover ,wait 3ms to deep sleep\n");
		usleep_range(3000,3100);
		fts_irq_disable();
		ret = fts_write_reg(FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP);
		FTS_INFO("enter_smart_cover success\n");
	}
...
static int tpd_get_smart_cover(struct ztp_device *cdev)
{
	...
	cdev->b_smart_cover_enable = ts_data->ztec.is_smart_cover;
...
int tpd_register_fw_class(struct fts_ts_data *data)
{
	...
	tpd_cdev->get_gesture = fts_get_wakegesture;
	tpd_cdev->wake_gesture = fts_enable_wakegesture;

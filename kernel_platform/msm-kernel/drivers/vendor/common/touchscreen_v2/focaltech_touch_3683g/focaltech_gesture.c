...
static void fts_gesture_report(struct fts_ts_data *ts_data, struct input_dev *input_dev, int gesture_id)
{
	...
	} else if (gesture_id == GESTURE_SINGLE && ts_data->ztec.is_single_aod) {
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    if(gesture->gesture_id == 36 || gesture->gesture_id == 37) {
        if (gesture->point_num > 1) {
            FTS_ERROR("gesture point num error, reset!");
...
int fts_gesture_suspend(struct fts_ts_data *ts_data)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (state == ENABLE) {
            FTS_INFO("ljk: FTS_REG_GESTURE_EN enable succeed!");
            break;
        }
        else {
            FTS_INFO("ljk: FTS_REG_GESTURE_EN enable failed!");
        }
...
int fts_gesture_resume(struct fts_ts_data *ts_data)
{
	...
	for (i = 0; i < 5; i++) {
		...
		fts_read_reg(FTS_REG_GESTURE_EN, &state);
		if (state == DISABLE) {
            FTS_INFO("ljk: FTS_REG_GESTURE_EN disable succeed!");
			break;
        }
        else {
            FTS_INFO("ljk: FTS_REG_GESTURE_EN disable failed!");
        }
	}

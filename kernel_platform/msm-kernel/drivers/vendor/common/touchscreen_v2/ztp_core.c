...
void tpd_zlog_record_notify(zlog_error_no error_no)
{
	...
	case TP_I2C_R_ERROR_NO:
		...
		TPD_ZLOG("tpd i2c read err,count:%lu. %s\n",
	...
	case TP_I2C_W_ERROR_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd i2c write err,count:%lu. %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_I2C_R_WARN_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd i2c read warnning,count:%lu.%s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_I2C_W_WARN_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd i2c write warnning,count:%lu.%s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_SPI_R_ERROR_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd SPI read err,count:%lu.%s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_SPI_W_ERROR_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd SPI write err,count:%lu.%s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_SPI_R_WARN_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd SPI read warnning,count:%lu.%s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_SPI_W_WARN_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd SPI write warnning,count:%lu.%s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_CRC_ERROR_NO:
		if ((tpd_zlog_check(error_no) < 0) || (after_reset_time < 200))
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd crc check err,count:%lu. %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_FW_UPGRADE_ERROR_NO:
		if (tpd_zlog_check(error_no) < 0)
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd firmware upgrade err,count:%lu. %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_REQUEST_FIRMWARE_ERROR_NO:
		if (tpd_zlog_check(error_no) < 0)
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd request firmware upgrade err,count:%lu. %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_ESD_CHECK_ERROR_NO:
		if (tpd_zlog_check(error_no) < 0)
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd esd check err,count:%lu. %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_ESD_CHECK_WARN_NO:
		if (tpd_zlog_check(error_no) < 0)
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd esd check warnning,count:%lu. %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
	...
	case TP_GHOST_ERROR_NO:
		if (tpd_zlog_check(error_no) < 0)
			break;
		cdev->zlog_item.timer[error_no] = jiffies;
		TPD_ZLOG("tpd ghost err,count:%lu. %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
		zlog_client_record(cdev->zlog_client, "tpd ghost err,count:%lu.\n %s\n",
			cdev->zlog_item.count[error_no], cdev->ztp_zlog_buffer);
					 ...
					 char __user *buffer, size_t count, loff_t *offset)
{
	...
	if (cdev->tp_firmware != NULL) {
		...
		pr_notice("tpd: get tp noise size:%zu.\n", cdev->tp_firmware->size);
	}
				...
				const char __user *buffer, size_t len, loff_t *off)
{
	...
	while (((token = strsep(&cur, ",")) != NULL) && (count < 10)) {
		ret = kstrtouint(token, 10, &s_to_u16);
		if (ret == 0) {
			data[count] = s_to_u16;
			count++;
		}
...
/* Started by AICoder, pid:m3a30b192e13e99147c509a980ebe4355746da16 */
static ssize_t tp_test_read(struct file *file,
					 char __user *buffer, size_t count, loff_t *offset)
{
	ssize_t len = 0;
	uint8_t data_buf[10] = {0};
	...
	if (cdev->ztp_probe_fail_chip_id != TS_CHIP_MAX) {
		ret = PROBE_TEST_ERR;
		TPD_DMESG("TP probe failed,chip_id = 0x%02x", cdev->ztp_probe_fail_chip_id);
		goto exit;
	} else {
		TPD_DMESG("TP probe success,chip_id = 0x%02x", cdev->tp_chip_id);
	}

	if (!cdev->fw_ready) {
		ret = FW_TEST_ERR;
		TPD_DMESG("err:cdev->fw_ready is false");
		goto exit;
	} else {
		TPD_DMESG("cdev->fw_ready is true");
	}
...
static ssize_t tp_test_write(struct file *file,
				const char __user *buffer, size_t len, loff_t *off)
{
	TPD_DMESG("reserved no use");
	return len;
}
/* Ended by AICoder, pid:m3a30b192e13e99147c509a980ebe4355746da16 */
...
static const struct proc_ops proc_ops_tp_test = {
	.proc_read = tp_test_read,
	.proc_write = tp_test_write,
};
...
static void create_tpd_proc_entry(void)
{
	...
	tpd_proc_entry = proc_create(PROC_TP_TEST, 0664, tpd_proc_dir, &proc_ops_tp_test);
	if (tpd_proc_entry == NULL)
		pr_err("proc_create tp_test failed!\n");
		...
		char *buf, loff_t pos, size_t count)
{
	...
	pr_info("tpd: cdev->fw_data_pos: %d, count:%zu\n", cdev->fw_data_pos, count);
	mutex_lock(&cdev->cmd_mutex);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (cdev->fw_data_pos + count > cdev->tp_firmware->size)
		count = cdev->tp_firmware->size - cdev->fw_data_pos;
	/*pr_info("tpd: cdev->fw_data_pos: %u, count:%d\n", cdev->fw_data_pos, count);*/
...
static void tp_ghost_check_work(struct work_struct *work)
{
	...
	if (tp_ghost_check()) {
		...
		if (cdev->ghost_rst_num < 3) {
			if (cdev->ghost_check_reset) {
				cdev->ghost_check_reset(cdev);
...
static int zte_touch_probe(struct platform_device *pdev)
{
	...
	if(tpd_workqueue_init())
		return -ENOMEM;
	queue_delayed_work(ztp_dev->tpd_wq, &ztp_dev->tpd_probe_work, msecs_to_jiffies(500));

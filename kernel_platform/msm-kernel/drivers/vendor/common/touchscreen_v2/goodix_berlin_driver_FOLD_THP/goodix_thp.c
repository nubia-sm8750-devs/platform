...
static long goodix_thp_ioctl_get_driver_state(struct goodix_thp_core *core_data, unsigned long arg)
{
        ...
        ts_info(ts_dev->dev, "%s:driver state = %d", __func__, suspend_status);
...
static int goodix_thp_power_init(struct goodix_thp_core *core_data)
{
        ...
        r = devm_gpio_request_one(dev, core_data->avdd_enable_gpio,
						GPIOF_OUT_INIT_LOW, "TP_AVDD_GPIO");
        if (r) {
        	ts_err(ts_dev->dev,"failed to request avdd_enable_gpio");
        	return r;
        }

        core_data->iovdd_enable_gpio = ts_bdata->iovdd_enable_gpio;
        r = devm_gpio_request_one(dev, core_data->iovdd_enable_gpio,
						GPIOF_OUT_INIT_LOW, "TP_IOVDD_GPIO");
        if (r) {
        	ts_err(ts_dev->dev,"failed to request iovdd_enable_gpio");
        	return r;
        }
...
static int goodix_thp_power_on(struct goodix_thp_core *core_data)
{
	...
	if (r) {
	        ts_err(ts_dev->dev,"Failed to enable avdd power:%d", r);
		goto power_off;
	}
        r = gpio_direction_output(core_data->iovdd_enable_gpio, 1);

	if (r) {
	        ts_err(ts_dev->dev,"Failed to enable vdd power:%d", r);
		goto power_off;
	}

	ts_info(ts_dev->dev,"regulator enable SUCCESS");
...
power_off:
	...
	gpio_direction_output(core_data->avdd_enable_gpio, 0);
        gpio_direction_output(core_data->iovdd_enable_gpio, 0);
        return r;
}
...
static int goodix_thp_gesture_irq_handler(struct goodix_thp_core *core_data)
{
        ...
        case 0xAB: // DOWN
                ...
                break;
        case 0x46: // FP_DOWN
                ts_info(ts_dev->dev, "get gesture event: finger print down");
                break;
        case 0x55: // FP_UP
                ts_info(ts_dev->dev, "get gesture event: finger print up");
                break;
...
static long goodix_thp_input_agent_ioctl_set_coordinate(struct goodix_thp_core *core_data, unsigned long arg)
{
        ...
        if (arg == 0) {
                ts_err(ts_dev->dev, "%s:arg is null.", __func__);
                return -EINVAL;
        }
        ...
        if (data.touch[STYLUS_TRACK_ID].touch_valid == 1) {
                ...
                input_sync(input_dev);
                // report stylus
                ts_debug(ts_dev->dev, "x:%d y:%d p:%d tilt_x:%d tilt_y:%d",
                ...
                if (data.ref_not_set == 0) {
                        for (i = 0; i < INPUT_AGENT_MAX_FINGERS; i++) {
                                ...
                                if (data.touch[i].touch_valid != 0) {
                                        ts_debug(ts_dev->dev, "[%d] x:%d y:%d w:%d", i, 
...
static int goodix_thp_input_agent_init(struct goodix_thp_core *core_data)
{
        ...
        input_dev->id.product = 0x0200 + core_data->pdev->id;
...
static int goodix_thp_suspend(struct goodix_thp_core *core_data)
{
		...
		ts_info(ts_dev->dev, "status is %d", status);
...
static void goodix_ts_register_for_panel_events(struct goodix_thp_core *core_data)
{
	...
	if (!cookie) {
		ts_err(ts_dev->dev, "Failed to register for panel events\n");
		return ;
	}

	ts_info(ts_dev->dev, "registered for panel notifications panel: 0x%p\n",
			active_panel);
...
static int tpd_goodix_ts_resume(void *_core_data)
{
	struct goodix_thp_core *core_data = (struct goodix_thp_core *)_core_data;
        ...
        ts_info(ts_dev->dev, "%s enter resume workqueue", __func__);
...
static int tpd_goodix_ts_suspend(void *_core_data)
{
	struct goodix_thp_core *core_data = (struct goodix_thp_core *)_core_data;
        ...
        ts_info(ts_dev->dev, "%s enter suspend workqueue", __func__);
...
static void tpd_resume_suspend_register(struct goodix_thp_core *core_data)
{
	...
	ts_info(ts_dev->dev, "tpd_resume_suspend_register IN");
...
static int goodix_ts_check_dt(struct device *dev)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (node != NULL)
			ts_info(dev, "%s: node = %s", __func__, node->name);
		...
		if (PTR_ERR(panel) == -ENODEV) {
			ts_info(dev, "%s: no device!", __func__);
			retval = -ENODEV;
		} else if (PTR_ERR(panel) == -EPROBE_DEFER) {
			ts_info(dev, "%s: device has not been probed yet", __func__);
			retval = -EPROBE_DEFER;
		}
...
static int goodix_thp_probe(struct platform_device *pdev)
{
	...
	ts_info(tdev->dev, "tp_report_rate set 120HZ");
#else
	core_data->ztec.tp_report_rate = 1;
	ts_info(tdev->dev, "tp_report_rate set 240HZ");
#endif
...
static int goodix_pm_suspend(struct device *dev)
{
	struct goodix_thp_core *core_data = dev_get_drvdata(dev);
        ...
        ts_info(tdev->dev, "system enters into pm_suspend");
...
static int goodix_pm_resume(struct device *dev)
{
	...
	ts_info(tdev->dev, "system resumes from pm_suspend");
	core_data->pm_suspend = false;

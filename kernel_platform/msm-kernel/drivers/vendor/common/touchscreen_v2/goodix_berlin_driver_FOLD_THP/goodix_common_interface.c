...
static int tpd_init_tpinfo(struct ztp_device *cdev)
{
	...
	if (atomic_read(&core_data->suspended)) {
		ts_err(ts_dev->dev, "%s: error, tp in suspend!", __func__);
		return -EIO;
	}

	if (atomic_cmpxchg(&ato_ver, 0, 1)) {
		ts_err(ts_dev->dev, "busy, wait!");
		return -EIO;
	}

	ts_info(ts_dev->dev, "%s: enter!", __func__);

	snprintf(cdev->ic_tpinfo.tp_name, 200, "thp_version   IC type:goodix GT9916\n%s",
				core_data->ts_dev->board_data.thp_ver);

	ts_info(ts_dev->dev, "%s: end!", __func__);
...
void goodix_thp_tpd_register_fw_class(struct goodix_thp_core *core_data)
{
	...
	ts_info(ts_dev->dev, "%s: entry", __func__);
#if 0
#ifdef GOODIX_USB_DETECT_GLOBAL

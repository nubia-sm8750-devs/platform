        ...
        struct goodix_thp_board_data *board_data)
{
        ...
        if (r < 0) {
                ts_err(dev, "invalid avdd-enable-gpio in dt: %d", r);
                return -EINVAL;
        }
        ts_info(dev, "get avdd-enable-gpio[%d] from dt", r);
        board_data->avdd_enable_gpio = r;

        r = of_get_named_gpio(node, "iovdd-enable-gpio", 0);
        if (r < 0) {
                ts_err(dev, "invalid iovdd-enable-gpio in dt: %d", r);
                return -EINVAL;
        }
        ts_info(dev, "get iovdd-enable-gpio[%d] from dt", r);

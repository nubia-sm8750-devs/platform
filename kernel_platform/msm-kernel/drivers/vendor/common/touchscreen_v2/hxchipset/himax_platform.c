...
int himax_parse_dt(struct himax_ts_data *ts, struct himax_platform_data *pdata)
{
    ...
    r = of_property_read_string(dt, "himax,avdd-name", &name_tmp);
    if (!r) {
        I("avdd name form dt: %s", name_tmp);
        if (strlen(name_tmp) < sizeof(pdata->avdd_name))
            strncpy(pdata->avdd_name,
                name_tmp, sizeof(pdata->avdd_name));
        else
            I("invalied avdd name length: %ld > %ld",
                strlen(name_tmp),
                sizeof(pdata->avdd_name));
        }

    r = of_property_read_string(dt, "himax,iovdd-name", &name_tmp);
	if (!r) {
		I("vdd name from dt: %s", name_tmp);
		if (strlen(name_tmp) < sizeof(pdata->iovdd_name))
			strncpy(pdata->iovdd_name,
				name_tmp, sizeof(pdata->iovdd_name));
		else
			I("invalied iovdd name length: %ld > %ld",
				strlen(name_tmp),
...
int himax_gpio_power_config(struct himax_ts_data *ts, struct himax_platform_data *pdata)
{
	I("%s: entered, CONFIG_HMX_DB is defined.\n", __func__);
	int error;
	/* struct i2c_client *client = hx_s_ts->client; */
...
int himax_gpio_power_config(struct himax_ts_data *ts, struct himax_platform_data *pdata)
{
	I("%s: entered, CONFIG_HMX_DB is not defined.\n", __func__);
	int error = 0;
	int ret = 0;
    ...
    if (strlen(pdata->iovdd_name)) {
		pdata->vdd = devm_regulator_get(dev,
			pdata->iovdd_name);
		if (IS_ERR_OR_NULL(pdata->vdd)) {
			ret = PTR_ERR(pdata->vdd);
			E("Failed to get regulator vdd:%d", ret);
			pdata->vdd = NULL;
		}
		if (ret < 0) {
			E("set vdd voltage failed");
			//return ret;
		}
	} else {
		I("vdd name is NULL");
	...
	if (strlen(pdata->avdd_name)) {
		...
		if (IS_ERR_OR_NULL(pdata->avdd)) {
			...
			E("Failed to get regulator avdd:%d", ret);
			pdata->avdd = NULL;
		}
		if (ret < 0) {
			E("set avdd voltage failed");
			//return ret;
		}
	} else {
		I("avdd name is NULL");
    ...
    if (ret) {
        E("Failed to enable iovdd:%d", ret);
    }
    I("vdd enable SUCCESS");
    usleep_range(3000, 3100);

    ret = regulator_enable(pdata->avdd);
    if (ret) {
        E("Failed to enable avdd:%d", ret);
    }
    I("avdd enable SUCCESS");
...
static void himax_notifier_register(struct work_struct *work)
{
	...
	if (g_hx_active_panel) {
		...
		I("%s:registered for panel notifications panel\n",
			__func__);

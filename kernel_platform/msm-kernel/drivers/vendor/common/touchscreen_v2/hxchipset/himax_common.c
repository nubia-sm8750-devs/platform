...
/*start ts_work*/
#if defined(HX_USB_DETECT_GLOBAL)
static bool himax_get_charger_status(void)
{
...
void himax_cable_detect_func(bool force_renew)
{
	...
	USB_detect_flag = himax_get_charger_status();
...
static void himax_point_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		I("%s:ABS_MT_POSITION_X = %d, ABS_MT_POSITION_Y = %d\n", __func__, g_target_report_data->p[i].x, g_target_report_data->p[i].y);
	}
#if !defined(HX_PROTOCOL_A)
	input_report_key(ts->input_dev, BTN_TOUCH, 1);
#endif

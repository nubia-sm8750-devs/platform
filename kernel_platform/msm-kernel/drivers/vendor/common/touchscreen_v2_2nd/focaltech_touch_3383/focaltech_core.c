...
#define FTS_VTG_MIN_UV                      3100000
#define FTS_VTG_MAX_UV                      3100000
...
#define LAGRE_SUPPRESSION_AREA "large_area2=true"
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    do {
        ...
        if ((ret < 0) || (0x0 == chip_id[0])) {
            FTS_INFO("chip_id[0] read invalid, read:0x%02x", chip_id[0]);
        } else {
            ret = fts_read_reg(FTS_REG_CHIP_ID2, &chip_id[1]);
            if ((ret < 0) || (0x0 == chip_id[1])) {
                FTS_INFO("chip_id[1] read invalid, read:0x%02x", chip_id[1]);
            } else {
...
void fts_release_all_finger(void)
{
	...
	if (tpd_cdev_2nd->zte_tp_algo_2nd) 
...
static int fts_input_report_b(struct fts_ts_data *ts_data, struct ts_event *events)
{
	...
	cancel_delayed_work_sync(&tpd_cdev_2nd->point_report_check_work);
	queue_delayed_work(tpd_cdev_2nd->tpd_report_wq, &tpd_cdev_2nd->point_report_check_work, msecs_to_jiffies(80));
#endif
...
#ifdef FTS_REPORT_BY_ZTE_ALGO
			if (tpd_cdev_2nd->zte_tp_algo_2nd) {
				tpd_touch_press_2nd(ts_data->input_dev, events[i].x, events[i].y, events[i].id,  events[i].area, events[i].p);
			} else {
...
#ifdef FTS_REPORT_BY_ZTE_ALGO
			if (tpd_cdev_2nd->zte_tp_algo_2nd) {
				tpd_touch_release_2nd(ts_data->input_dev, events[i].id);
			} else {
				input_mt_slot(input_dev, events[i].id);
				input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, false);
			}
...
#ifdef FTS_REPORT_BY_ZTE_ALGO
				if (tpd_cdev_2nd->zte_tp_algo_2nd) {
					tpd_touch_release_2nd(ts_data->input_dev, i);
				} else {
...
static int fts_input_report_touch_pv2(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    for (i = 0; i < event_num; i++) {
        ...
        events[i].y = ((touch_buf[FTS_TOUCH_OFF_ID_YH + base] & 0x0F) << 12) \
                      + ((touch_buf[FTS_TOUCH_OFF_YL + base] & 0xFF) << 4) \
                      + (touch_buf[FTS_TOUCH_OFF_PRE + base] & 0x0F);
        //FTS_INFO("before:P%d(%d, %d)", events[i].id, events[i].x, events[i].y);
#if FTS_TOUCH_HIRES_EN
        events[i].x = (events[i].x * FTS_TOUCH_HIRES_X ) / FTS_HI_RES_X_MAX;
        events[i].y = (events[i].y * FTS_TOUCH_HIRES_X ) / FTS_HI_RES_X_MAX;
#else
        ...
        //FTS_INFO("after:P%d(%d, %d)", events[i].id, events[i].x, events[i].y);
    }
...
/* Started by AICoder, pid:v0696h6b2cx471d14ae80aca6016651d3a94baf8 */
...
/* Ended by AICoder, pid:v0696h6b2cx471d14ae80aca6016651d3a94baf8 */
...
static irqreturn_t fts_irq_handler(int irq, void *data)
{
    ...
    if (tpd_cdev_2nd->bbat_test_enter) {
        if (tpd_cdev_2nd->bbat_int_test == false) {
            tpd_cdev_2nd->bbat_int_test = true;
            complete(&tpd_cdev_2nd->bbat_test_completion);
            FTS_INFO("tpd int BBAT test success");
        }
        return IRQ_HANDLED;
    }
    ...
    if (ts_data->suspended) {
        ...
        pm_wakeup_event(ts_data->dev, jiffies_to_msecs(FTS_WAKELOCK_TIMEOUT));
        FTS_INFO("pm_wakeup_event jiffies_to_msecs(5000)");
#else
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
    ...
    input_dev->phys = "secondary_tp";
    input_dev->id.vendor = 0x19D2;
    input_dev->id.product = 0x19D2;
    input_dev->id.version = 0x42;
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            fts_set_reset(ts_data, 0);
            fts_msleep(2);
            FTS_INFO("set power to on dd");

            //ret = regulator_enable(ts_data->vdd);
            gpio_set_value(ts_data->pdata->avdd_gpio, 1);
            fts_msleep(2);
            gpio_set_value(ts_data->pdata->dvdd_gpio, 1);

            if (ret) {
                FTS_ERROR("enable vdd regulator failed, ret=%d", ret);
            } else {
                FTS_INFO("enable vdd regulator success");
            }
        ...
        if (!ts_data->power_disabled) {
            ...
            if (gpio_is_valid(ts_data->pdata->avdd_gpio)) {
                gpio_set_value(ts_data->pdata->avdd_gpio, 0);
                FTS_INFO("set avdd to low");
            }
            fts_msleep(2);
            if (gpio_is_valid(ts_data->pdata->dvdd_gpio)) {
                gpio_set_value(ts_data->pdata->dvdd_gpio, 0);
                FTS_INFO("set dvdd to low");
            }
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
    ...
    if (IS_ERR_OR_NULL(ts_data->vdd)) {
        ...
        FTS_INFO("get vdd regulator success");
    }
    ...
    if (regulator_count_voltages(ts_data->vdd) > 0) {
        ...
        if (ret) {
            ...
            FTS_INFO("vdd regulator set_vtg success");
        }
    }*/
    ...
    if (gpio_is_valid(ts_data->pdata->avdd_gpio)) {
        ...
        if (ret) {
            ...
            ts_data->pdata->avdd_gpio = 0;
            return ret;
        } else {
            FTS_INFO("[GPIO]avdd gpio request success");
        }
    ...
    if (gpio_is_valid(ts_data->pdata->dvdd_gpio)) {
        ...
        if (ret) {
            ...
            ts_data->pdata->dvdd_gpio = 0;
            return ret;
        } else {
            FTS_INFO("[GPIO]dvdd gpio request success");
        }
        gpio_direction_output(ts_data->pdata->dvdd_gpio, 0);
    }
    ...
    if (ret) {
        FTS_ERROR("fail to enable power(regulator or gpio)");
    } else {
        FTS_INFO("success to enable power(regulator or gpio)");
    }
...
static int fts_power_source_exit(struct fts_ts_data *ts_data)
{
    ...
    if (gpio_is_valid(ts_data->pdata->dvdd_gpio)) {
        gpio_free(ts_data->pdata->dvdd_gpio);
    }
...
static int fts_power_init(struct fts_ts_data *ts_data)
{
    ...
    if (ret) {
        FTS_ERROR("fail to get power(regulator or gpio)");
        return ret;
    } else {
        FTS_INFO("success to get power(regulator or gpio)");
    }
#else
#if (!FTS_CHIP_IDC)
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    if (pdata->avdd_gpio < 0) {
        ...
        FTS_INFO("avdd gpio:%d", pdata->avdd_gpio);
    }

    pdata->dvdd_gpio = of_get_named_gpio(np, "focaltech,dvdd-gpio", 0);
    if (pdata->dvdd_gpio < 0) {
        FTS_ERROR("Unable to get dvdd_gpio");
    } else {
        FTS_INFO("dvdd gpio:%d", pdata->dvdd_gpio);
    }
#endif
...
int fts_ts_suspend(struct device *dev)
{
    ...
    status = (ts_data->ztec.is_wakeup_gesture) | ts_data->ztec.is_single_tap;
    if (status)
...
/* Started by AICoder, pid:yf7eev08c1r83f914fe608c1e03a1d9077924177 */
static void fts_ts_register_for_panel_events(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("registered for panel notifications panel success\n");
...
static int drm_check_dt(struct device_node *np)
{
    ...
    for (i = 0; i < count; i++) {
        node = of_parse_phandle(np, "panel_2nd", i);
        panel = of_drm_find_panel(node);
        of_node_put(node);
        if (!IS_ERR(panel)) {
            FTS_INFO("find drm_panel_2nd successfully");
            active_panel = panel;
            return 0;
        }
    }
...
/* Ended by AICoder, pid:yf7eev08c1r83f914fe608c1e03a1d9077924177 */
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    for (time = 0; time < 10; time++) {
        if (ret) {
            FTS_ERROR("parse drm-panel fail, retry :%d!", time);
    ...
    ts_data->ts_workqueue = create_singlethread_workqueue("fts_i2c_wq");
    if (!ts_data->ts_workqueue) {
        FTS_ERROR("create fts_i2c workqueue fail");
    } else {
    ...
    wakeup_source_init(&ts_data->ws, "fts_i2c_ws");
    ts_data->p_ws = &ts_data->ws;
#elif (LINUX_VERSION_CODE > KERNEL_VERSION(6, 1, 0))
    device_init_wakeup(ts_data->dev, true);  // add wakeup node
    FTS_INFO("device_init_wakeup true");
#else
    ts_data->p_ws = wakeup_source_register(ts_data->dev, "fts_i2c_ws");
#endif
...
err_bus_init:
    ...
    ts_data->p_ws = NULL;
#elif (LINUX_VERSION_CODE > KERNEL_VERSION(6, 1, 0))
    device_init_wakeup(ts_data->dev, false);  // remove wakeup node
    FTS_INFO("device_init_wakeup false");
#else

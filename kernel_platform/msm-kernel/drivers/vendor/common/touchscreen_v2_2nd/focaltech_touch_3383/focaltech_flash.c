...
#define FTS_FW_NAME_PREX_WITH_REQUEST               "focaltech_ts_fw_2nd_"
...
int fts_fwupg_get_moduleid(struct fts_upgrade *upg, int *module_id)
{
    int ret = 0;
    bool fwvalid = false;
    u8 vendor_id = 0;
...
#endif
        i = get_fts_module_info_from_lcd();
        if (i > (ARRAY_SIZE(module_list) - 1)) {
            i = ARRAY_SIZE(module_list) - 1;
        }
        info = &module_list[i];
    }

    upg->module_info = info;
    FTS_INFO("upg->module_info->vendor_name = %s", upg->module_info->vendor_name);

...
static void fts_gesture_report(struct fts_ts_data *ts_data, struct input_dev *input_dev, int gesture_id)
{
    ...
    case GESTURE_DOUBLECLICK:
        gesture = GESTURE_DOUBLECLICK;
        break;
    case GESTURE_SINGLECLICK:
        gesture = GESTURE_SINGLECLICK;
        break;
    case GESTURE_O:
    ...
    /* report event key */
	if (gesture_id == GESTURE_SINGLECLICK && ts_data->ztec.is_single_aod) {
		if (tpd_cdev_2nd->tpd_report_uevent_2nd != NULL) {
			FTS_INFO("Single Click Uevent");
			tpd_cdev_2nd->tpd_report_uevent_2nd(single_tap2);
		}
	}

	if (gesture == GESTURE_DOUBLECLICK && ts_data->ztec.is_wakeup_gesture) {
		FTS_DEBUG("Gesture Code=%d", gesture);
		if (tpd_cdev_2nd->tpd_report_uevent_2nd != NULL) {
			FTS_INFO("Double Click Uevent");
			tpd_cdev_2nd->tpd_report_uevent_2nd(double_tap2);
		}
	}
}

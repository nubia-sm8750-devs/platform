...
bool fts_compare_array(int *data, int *min, int *max, bool key)
{
    int i = 0;
    bool result = true;
    struct fts_test *tdata = fts_ftest;
...
/*
 * fts_show_data - show and save test data to testresult.txt
 */
void fts_show_data(int *data, bool key)
{
#if TXT_SUPPORT
    int i = 0;
    int j = 0;
...
{
	FTS_TEST_FUNC_ENTER();
	tpd_copy_to_tp_firmware_data_2nd(data_buf);
	FTS_TEST_FUNC_EXIT();
	return 0;
}
...
static ssize_t fts_test_show_i2c(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t fts_test_store_i2c(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = 0;
...
*/
static DEVICE_ATTR(fts_test_i2c, S_IRUGO | S_IWUSR, fts_test_show_i2c, fts_test_store_i2c);

static struct attribute *fts_test_attributes_i2c[] = {
    &dev_attr_fts_test_i2c.attr,
    NULL
};

static struct attribute_group fts_test_attribute_group_i2c = {
    .attrs = fts_test_attributes_i2c
};
...
int fts_test_init(struct fts_ts_data *ts_data)
{
    ...
    ret = sysfs_create_group(&ts_data->dev->kobj, &fts_test_attribute_group_i2c);
    if (0 != ret) {
        FTS_TEST_ERROR("sysfs(test) create fail");
        sysfs_remove_group(&ts_data->dev->kobj, &fts_test_attribute_group_i2c);
    } else {
        FTS_TEST_DBG("sysfs(test) create successfully");
    }

    fts_ftest->proc_csv = proc_create_data("fts_test_csv_i2c", 0777, NULL, &fts_proccsv_fops, fts_ftest);

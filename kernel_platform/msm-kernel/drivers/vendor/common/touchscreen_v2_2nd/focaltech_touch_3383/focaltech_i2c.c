...
extern int fts_tpd_register_fw_class(struct fts_ts_data *data);
...
static int fts_ts_probe(struct i2c_client *client)
{
    ...
    ts_data->ztec.is_wakeup_gesture = 1;
    ts_data->ztec.is_palm_mode = 1;
    ...
    fts_tpd_register_fw_class(ts_data);
    tpd_cdev_2nd->TP_have_registered = true;
    tpd_cdev_2nd->tp_chip_id_2nd = TS_CHIP_FOCAL;
...
int fts_ts_init_2nd(void)
{
    ...
    FTS_FUNC_ENTER();
    FTS_INFO("2024-10-22_2");

	ret =  get_tp_chip_id_2nd();
	if (ret == 0) {
		if ((tpd_cdev_2nd->tp_chip_id_2nd != TS_CHIP_MAX) && (tpd_cdev_2nd->tp_chip_id_2nd != TS_CHIP_FOCAL)) {
...
//module_init(fts_ts_init_2nd);
//module_exit(fts_ts_exit_2nd);

...
static enum hrtimer_restart fan_hrtimer_hander(struct hrtimer *timer)
{
	...
	if(chip->fan->cal_stat == CAL_STAT_PROCING){
		...
		disable_irq_nosync(chip->fan->speed_irq);
...
static int fan_level_set(struct fan_dev *fan, u8 level)
{
	...
	/* Started by AICoder, pid:ce2790ef92137e414e3a0af52056360eaf813ffa */
	pstate.duty_cycle = 5500 * level;
...
{
/* Started by AICoder, pid:te7cbjcb76hb0b4149b808ec003eb40fc72597ae */
	bool enable = false;
	int tmpenable = 0;

	sscanf(buf, "%d", &tmpenable);
	enable = !!tmpenable;
/* Ended by AICoder, pid:te7cbjcb76hb0b4149b808ec003eb40fc72597ae */
...
static int nb_fan_parse_dt(struct nb_chip *chip)
{
	...
	int rc = 0, fan_count = 0;

	fan_count = of_get_available_child_count(node);
	if (fan_count <= 0) {
		FAN_DBG("No fan child node defined\n");
		return -ENODEV;
	}

	if (fan_count > FAN_COUNT_MAX) {
		FAN_DBG("Can't support %d fans(max %d)\n",fan_count, FAN_COUNT_MAX);
		return -EINVAL;
	}
	...
	*/
	...
	chip->fan = devm_kcalloc(chip->dev, fan_count, sizeof(struct fan_dev), GFP_KERNEL);
	if (!chip->fan) {
		FAN_DBG("Failed to allocate memory for fan_dev\n");
		return -ENOMEM;
	}
...
static int nb_fan_probe(struct platform_device *pdev)
{
	...
	FAN_DBG("Chip(fan)probe finished!\n");
...
MODULE_DEVICE_TABLE(of, nubia_fan_of_match);
...
MODULE_SOFTDEP("pre: leds-qcom-lpg");

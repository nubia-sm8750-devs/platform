...
void zsw_pm_resume_calculate_wakepercent(void)
{
	...
	for (j = 0; j < ARRAY_SIZE(subsystems_zte); j++) {
		if (1 == subsystems_zte[j].used) {
			...
			pr_info("[PM_V] calculate_wakepercent 1 delta=%llu subsys_delta=%llu s percent_wake=%u name=%s Ap_sleep=%llu subsys_flag=%d, mask=%d\n",
					delta, subsys_delta, (int)percent_wake, subsystem->name,
	...
	if ((sleep_zswsubsys_flag & ZSW_MODEM_CRASH_FLAG) != 0) {
		...
		if ((sleep_zswsubsys_flag & ZSW_ADSP_CRASH_FLAG) == 0) {
			pr_info("[PM_V] calculate_wakepercent a modem crash sleep_zswsubsys_flag=%d\n", sleep_zswsubsys_flag);
			sleep_zswsubsys_flag = 0;
			sleep_zswresumeparam_mask = ZSW_MODEM_CRASH;
		}
		...
		if ((sleep_zswsubsys_flag & ZSW_SLPI_CRASH_FLAG) == 0) {
			pr_info("[PM_V] calculate_wakepercent s modem crash sleep_zswsubsys_flag=%d\n", sleep_zswsubsys_flag);
			sleep_zswsubsys_flag = 0;
			sleep_zswresumeparam_mask = ZSW_MODEM_CRASH;
		}
	}
    ...
    if (sleep_zswaod_state == 1) {
        ...
        if ((sleep_zswsubsys_flag & ZSW_ADSP_CRASH_FLAG) != 0) {
            if ((sleep_zswsubsys_flag & ZSW_MODEM_CRASH_FLAG) == 0) {
                pr_info("[PM_V] calculate_wakepercent adsp crash sleep_zswsubsys_flag=%d\n", sleep_zswsubsys_flag);
                sleep_zswsubsys_flag = 0;
                sleep_zswresumeparam_mask = ZSW_ADSP_CRASH;
            }
        }
        ...
        if ((sleep_zswsubsys_flag & ZSW_SLPI_CRASH_FLAG) != 0) {
            if ((sleep_zswsubsys_flag & ZSW_MODEM_CRASH_FLAG) == 0) {
                pr_info("[PM_V] calculate_wakepercent slpi crash sleep_zswsubsys_flag=%d\n", sleep_zswsubsys_flag);
                sleep_zswsubsys_flag = 0;
                sleep_zswresumeparam_mask = ZSW_SLPI_CRASH;
            }
        }
	...
	if ((sleep_zswsubsys_flag & ZSW_CDSP_CRASH_FLAG) != 0) {
		pr_info("[PM_V] calculate_wakepercent cdsp crash sleep_zswsubsys_flag=%d\n", sleep_zswsubsys_flag);
		sleep_zswsubsys_flag = 0;
		sleep_zswresumeparam_mask = ZSW_CDSP_CRASH;
	}
...
void pm_subsystems_init(void)
{
    ...
    pr_info("pm_subsystems_init %d, %d\n", (int)ARRAY_SIZE(subsystems_str_8550), (int)ARRAY_SIZE(subsystems_str_default));

...
static long veb_ioctl_duplex(veb_private_data_t *priv, a5_duplex_con *con)
{
	...
	if (left) {
		...
		xfer_left.delay.value = 3;
		xfer_left.delay.unit = SPI_DELAY_UNIT_USECS;
...
static int veb_a5_spi_probe(struct spi_device *spi)
{
	...
	VEB_TRACE_IN();
#if defined(ZTE_CLOSE_VEB_A5)
	VEB_ERR("disable veb_a5, exit init\n");
	return -ENOMEM;
#endif
	...
	tmp_xfer = (unsigned char *)__get_free_pages(GFP_KERNEL, get_order(64 * 1024));
	if (tmp_xfer == NULL) {
		VEB_ERR("get_free_pages 64 * 1024 failed!\n");
		return -ENOMEM;
	}
...
/* Started by AICoder, pid:8eeeay20ccv81f114aa409ee10c7651a9ba36a7f */
void veb_a5_spi_remove(struct spi_device *spi)
{
    veb_private_data_t *priv = NULL;
    ...
    kfree(priv);
    priv = NULL;
    free_pages((unsigned long)tmp_xfer, get_order(64 * 1024));
    tmp_xfer = NULL;

    veb_platform_exit(spi);
}
/* Ended by AICoder, pid:8eeeay20ccv81f114aa409ee10c7651a9ba36a7f */

...
static struct zlog_mod_info zlog_sc_dev = {
	.module_no = ZLOG_MODULE_SC,
	.name = "safechip",
	.device_name = "veb_a5",
	.ic_name = "veb_a5",
	.module_name = "SC",
...
struct zlog_client *zlog_sc_client = NULL;

void veb_zlog_register(void)
{
	if (zlog_sc_client) {
		VEB_INF("safechip zlog already registered, no need register again!\n");
		return;
	}

	zlog_sc_client = zlog_register_client(&zlog_sc_dev);
	if (!zlog_sc_client) {
		VEB_ERR("zlog_register_safechip_client fail\n");
	} else {
		VEB_INF("zlog_register_safechip_client success\n");
	}
}

void veb_zlog_unregister(void)
{
	if (zlog_sc_client) {
		zlog_unregister_client(zlog_sc_client);
		VEB_INF("zlog_unregister_safechip_client zlog_sc_dev\n");
	} else {
		return;
	}
}

void veb_zlog_record_notify(int errorno)
{
	if (!zlog_sc_client) {
		VEB_ERR("safechip zlog not registered!\n");
		return;
	}
	switch (errorno) {
	case ZLOG_SC_REQUEST_RST_GPIO_ERROR_NO:
		zlog_client_record(zlog_sc_client, "Failed to request veb-a5 rst gpio\n");
		zlog_client_notify(zlog_sc_client, ZLOG_SC_REQUEST_RST_GPIO_ERROR_NO);
		break;
	case ZLOG_SC_REQUEST_PWR_GPIO_ERROR_NO:
		zlog_client_record(zlog_sc_client, "Failed to request veb-a5 pwr gpio\n");
		zlog_client_notify(zlog_sc_client, ZLOG_SC_REQUEST_PWR_GPIO_ERROR_NO);
		break;
	case ZLOG_SC_REQUEST_STAT_GPIO_ERROR_NO:
		zlog_client_record(zlog_sc_client, "Failed to request veb-a5 stat gpio\n");
		zlog_client_notify(zlog_sc_client, ZLOG_SC_REQUEST_STAT_GPIO_ERROR_NO);
		break;
	case ZLOG_SC_REQUEST_WAKE_GPIO_ERROR_NO:
		zlog_client_record(zlog_sc_client, "Failed to request veb-a5 wake gpio\n");
		zlog_client_notify(zlog_sc_client, ZLOG_SC_REQUEST_WAKE_GPIO_ERROR_NO);
		break;
	case ZLOG_SC_MEMORY_KMALLOC_ERROR_NO:
		zlog_client_record(zlog_sc_client, "Failed to request veb-a5 wake gpio\n");
		zlog_client_notify(zlog_sc_client, ZLOG_SC_MEMORY_KMALLOC_ERROR_NO);
		break;
	default:
		break;
	}
...
void veb_qual_pin_init(struct spi_device *spi)
{
	...
	if (spi->dev.of_node) {
		reset_gpio = of_get_named_gpio(spi->dev.of_node, "veb_spi_vebv3,reset-gpio", 0);
		if (reset_gpio < 0) {
			VEB_ERR("falied to get rst gpio!\n");
			rc = -1;
		}
		VEB_INF("get reset_gpio, gpio = %d\n", reset_gpio);
		rc = devm_gpio_request(dev, reset_gpio, "veb_rst");
		if (rc) {
			VEB_ERR("failed to request reset gpio, rc = %d\n", rc);
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
			errorno = ZLOG_SC_REQUEST_RST_GPIO_ERROR_NO;
			veb_zlog_record_notify(errorno);
#endif
		}

		power_en_gpio = of_get_named_gpio(spi->dev.of_node, "veb_spi_vebv3,power-en-gpio", 0);
		if (power_en_gpio < 0) {
			VEB_ERR("falied to get pwr gpio!\n");
			rc = -1;
		}
		VEB_INF("get power_en_gpio, gpio = %d\n", power_en_gpio);
		rc = devm_gpio_request(dev, power_en_gpio, "veb_pwr");
		if (rc) {
			VEB_ERR("failed to request power gpio, rc = %d\n", rc);
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
			errorno = ZLOG_SC_REQUEST_PWR_GPIO_ERROR_NO;
			veb_zlog_record_notify(errorno);
#endif
		}

		status_gpio = of_get_named_gpio(spi->dev.of_node, "veb_spi_vebv3,status-gpio", 0);
		if (status_gpio < 0) {
			VEB_ERR("falied to get stat gpio!\n");
			rc = -1;
		}
		VEB_INF("get status_gpio, gpio = %d\n", status_gpio);
		rc = devm_gpio_request(dev, status_gpio, "veb_stat");
		if (rc) {
			VEB_ERR("failed to request status gpio, rc = %d\n", rc);
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
			errorno = ZLOG_SC_REQUEST_STAT_GPIO_ERROR_NO;
			veb_zlog_record_notify(errorno);
#endif
		}

		wake_gpio = of_get_named_gpio(spi->dev.of_node, "veb_spi_vebv3,wake-gpio", 0);
		if (wake_gpio < 0) {
			VEB_ERR("falied to get wake gpio!\n");
			rc = -1;
		}
		VEB_INF("get wake_gpio, gpio = %d\n", wake_gpio);
		rc = devm_gpio_request(dev, wake_gpio, "veb_wake");
		if (rc) {
			VEB_ERR("failed to request wake gpio, rc = %d\n", rc);
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
			errorno = ZLOG_SC_REQUEST_WAKE_GPIO_ERROR_NO;
			veb_zlog_record_notify(errorno);
#endif
		}
	}

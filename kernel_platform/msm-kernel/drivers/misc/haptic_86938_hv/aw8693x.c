...
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8693X_REG_SYSCTRL4,
					 AW8693X_BIT_SYSCTRL4_EN_DLL_MASK,
					 AW8693X_BIT_SYSCTRL4_EN_DLL_ON);
		i2c_w_bits(aw_haptic, AW8693X_REG_I2SCFG1,
			     ...
			     struct snd_soc_dai *dai)
{
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_I2SCFG2, mask, reg_val);
...
static int aw8693x_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		i2c_r_bytes(aw_haptic, AW8693X_REG_GLBRD5, &reg_val, AW_I2C_BYTE_ONE);
		if (reg_val != 0x0a) {
...
static int aw8693x_codec_probe(aw_snd_soc_codec_t *codec)
{
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_I2SCFG1, reg_val, AW_I2C_BYTE_TWO);
...
static void aw8693x_reg_unlock(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		...
		i2c_w_bytes(aw_haptic, AW8693X_REG_TMCFG, &reg_val, AW_I2C_BYTE_ONE);
	} else {
		/* Lock register */
...
static int aw8693x_check_qualify(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_EFCFG6, &reg_val, AW_I2C_BYTE_ONE);
	if (!(reg_val & 0x80)) {
...
{
	i2c_w_bytes(aw_haptic, AW8693X_REG_PLAYCFG2, &gain, AW_I2C_BYTE_ONE);
}
...
static void aw8693x_set_bst_vol(struct aw_haptic *aw_haptic, uint8_t bst_vol)
{
//	uint8_t reg_val = 0;
...
//	aw_info("set bst vol = %u mV, reg_val = 0x%02x", bst_vol, reg_val);
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_PLAYCFG1,
				 AW8693X_BIT_PLAYCFG1_BST_VOUT_VREFSET_MASK, bst_vol);
}

static void aw8693x_set_wav_seq(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t seq)
{
	i2c_w_bytes(aw_haptic, AW8693X_REG_WAVCFG1 + wav, &seq, AW_I2C_BYTE_ONE);
}
...
static void aw8693x_set_wav_loop(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t loop)
{
	if (wav % 2) {
		loop <<= 0;
		i2c_w_bits(aw_haptic, AW8693X_REG_WAVCFG9 + (wav / 2),
...
{
	i2c_w_bytes(aw_haptic, AW8693X_REG_RTPDATA, data, len);
}

static void aw8693x_set_rtp_aei(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8693X_REG_SYSINTM,
...
static void aw8693x_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_RAMADDRH, ram_addr, AW_I2C_BYTE_TWO);
}
...
static void aw8693x_set_base_addr(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_RTPCFG1,
				 AW8693X_BIT_RTPCFG1_BASE_ADDR_H_MASK, rtp_addr[0]);
	i2c_w_bytes(aw_haptic, AW8693X_REG_RTPCFG2, &rtp_addr[1], AW_I2C_BYTE_ONE);
}

static void aw8693x_auto_brake_mode(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8693X_REG_PLAYCFG3,
...
{
	if (flag) {
		i2c_w_bits(aw_haptic, AW8693X_REG_CONTCFG1,
...
static uint8_t aw8693x_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	// 	aw_haptic->i2c->addr = 0x5B;
	i2c_r_bytes(aw_haptic, AW8693X_REG_GLBRD5, &state, AW_I2C_BYTE_ONE);
	// if (aw_haptic->tiktap_mix_flag)
...
static void aw8693x_play_go(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		...
		i2c_w_bytes(aw_haptic, AW8693X_REG_PLAYCFG4, &reg_val, AW_I2C_BYTE_ONE);
	} else {
...
static void aw8693x_vbat_ref_config(struct aw_haptic *aw_haptic, uint16_t vbat_ref)
{
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_DETCFG1,
...
static void aw8693x_vbat_mode_config(struct aw_haptic *aw_haptic, uint8_t flag)
{
	if (flag == AW_CONT_VBAT_HW_COMP_MODE) {
		i2c_w_bits(aw_haptic, AW8693X_REG_VBATCTRL,
...
{
	i2c_w_bits(aw_haptic, AW8693X_REG_SYSCTRL3,
...
static void aw8693x_upload_lra(struct aw_haptic *aw_haptic, uint32_t flag)
{
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_TRIMCFG5, &cali_data, AW_I2C_BYTE_ONE);
}
...
static uint8_t aw8693x_get_trim_lra(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_TRIMCFG5, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw8693x_protect_config(struct aw_haptic *aw_haptic, uint8_t prtime, uint8_t prlvl)
{
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_PWMCFG1,
	...
	if (prlvl != 0) {
		...
		i2c_w_bytes(aw_haptic, AW8693X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
		i2c_w_bytes(aw_haptic, AW8693X_REG_PWMCFG4, &prtime, AW_I2C_BYTE_ONE);
	} else {
		/* Disable */
		aw_info("disable protection mode");
		i2c_w_bits(aw_haptic, AW8693X_REG_PWMCFG3,
...
static void aw8693x_cont_config(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_CONTCFG6,
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG7,
			     &aw_haptic->info.cont_drv2_lvl, AW_I2C_BYTE_ONE);
	/* i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG8, &drv1_time, AW_I2C_BYTE_ONE); */
	i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG9, &drv2_time, AW_I2C_BYTE_ONE);
...
static void aw8693x_one_wire_init(struct aw_haptic *aw_haptic)
{
	...
	reg_val = 0x6C;
	i2c_w_bytes(aw_haptic, AW8693X_REG_GLBCFG4, &reg_val, AW_I2C_BYTE_ONE);

	reg_val = AW8693X_BIT_START_DLY_2P5MS;
	i2c_w_bytes(aw_haptic, AW8693X_REG_GLBCFG2, &reg_val, AW_I2C_BYTE_ONE);
	i2c_w_bits(aw_haptic, AW8693X_REG_TRGCFG8,
...
static void aw8693x_trig1_param_config(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_TRGCFG7,
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_TRGCFG1, &trig_config, AW_I2C_BYTE_ONE);
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_TRGCFG4, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8693x_trig2_param_config(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_TRGCFG2, &trig_config, AW_I2C_BYTE_ONE);
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_TRGCFG5, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8693x_trig3_param_config(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_TRGCFG3, &trig_config, AW_I2C_BYTE_ONE);
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_TRGCFG6, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8693x_interrupt_setup(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
...
static ssize_t aw8693x_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	int i = 0;
	int size = 0;
	...
	while (i < aw_haptic->ram.len) {
		...
		i2c_r_bytes(aw_haptic, AW8693X_REG_RAMDATA, buf + i, size);
...
static void aw8693x_get_first_wave_addr(struct aw_haptic *aw_haptic, uint8_t *wave_addr)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_RAMDATA, reg_val, AW_I2C_BYTE_THREE);
...
static void aw8693x_get_wav_seq(struct aw_haptic *aw_haptic, uint32_t len)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_WAVCFG1, reg_val, len);
	for (i = 0; i < len; i++)
...
static size_t aw8693x_get_wav_loop(struct aw_haptic *aw_haptic, char *buf)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_WAVCFG9, reg_val, AW_SEQUENCER_LOOP_SIZE);
...
static uint8_t aw8693x_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
	reg_val >>= 7;
...
static int aw8693x_read_f0(struct aw_haptic *aw_haptic)
{
	...
	/* lra_f0 */
	i2c_r_bytes(aw_haptic, AW8693X_REG_CONTRD14, reg_val, AW_I2C_BYTE_TWO);
...
#else
	/* cont_f0 */
	i2c_r_bytes(aw_haptic, AW8693X_REG_CONTRD16, reg_val, AW_I2C_BYTE_TWO);
	f0_reg = (reg_val[0] << 8) | reg_val[1];
	if (!f0_reg) {
		aw_haptic->f0 = 0;
	...
	aw_haptic->f0 = AW8693X_F0_FORMULA(f0_reg);
	aw_haptic->cont_f0 = aw_haptic->f0;
...
static int aw8693x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_DETCFG2,
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_CONTCFG6,
				 ...
				 (aw_haptic->info.is_enabled_track_en << 7));
	/* enable auto break */
	i2c_r_bytes(aw_haptic, AW8693X_REG_PLAYCFG3, &reg_val, AW_I2C_BYTE_ONE);
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG7, cont_config, AW_I2C_BYTE_THREE);
	/* TRACK_MARGIN */
	if (!aw_haptic->info.cont_track_margin) {
		aw_err("aw_haptic->info.cont_track_margin = 0");
	} else {
		i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG11,
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG2, cont_config, AW_I2C_BYTE_TWO);
...
static int aw8693x_ram_get_f0(struct aw_haptic *aw_haptic)
{
	...
	brk_en_default = reg_val & (~AW8693X_BIT_PLAYCFG3_BRK_EN_MASK);
	aw8693x_auto_brake_mode(aw_haptic, true);
	aw8693x_set_bst_vol(aw_haptic, 0x0D);
...
static uint8_t aw8693x_rtp_get_fifo_afs(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw8693x_get_osc_status(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_SYSST2, &state, AW_I2C_BYTE_ONE);
...
static void aw8693x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic,  AW8693X_REG_DETCFG1, &reg_val, AW_I2C_BYTE_ONE);
	adc_fs_default = reg_val & (~AW8693X_BIT_DETCFG1_ADC_FS_MASK);
	i2c_r_bytes(aw_haptic, AW8693X_REG_DETCFG2, &reg_val, AW_I2C_BYTE_ONE);
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_DETRD1, reg_array, AW_I2C_BYTE_TWO);
...
static void aw8693x_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	aw8693x_stop(aw_haptic);
	aw8693x_ram_init(aw_haptic, true);
	i2c_w_bits(aw_haptic, AW8693X_REG_DETCFG2, AW8693X_BIT_DETCFG2_DET_SEQ0_MASK,
				 AW8693X_BIT_DETCFG2_DET_SEQ0_VBAT);
	i2c_w_bits(aw_haptic, AW8693X_REG_DETCFG1, AW8693X_BIT_DETCFG1_DET_GO_MASK,
				 AW8693X_BIT_DETCFG1_DET_GO_ON);
	usleep_range(3000, 3500);
	i2c_w_bits(aw_haptic, AW8693X_REG_DETCFG1, AW8693X_BIT_DETCFG1_DET_GO_MASK,
				 AW8693X_BIT_DETCFG1_DET_GO_OFF);
...
static ssize_t aw8693x_get_reg(struct aw_haptic *aw_haptic, ssize_t len, char *buf)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_RSTCFG, reg_array, AW8693X_REG_RTPDATA);
	i2c_r_bytes(aw_haptic, (AW8693X_REG_RTPDATA + 1),
			    &reg_array[AW8693X_REG_RTPDATA + 1],
			    (AW8693X_REG_RAMDATA - AW8693X_REG_RTPDATA - 1));
	i2c_r_bytes(aw_haptic, (AW8693X_REG_RAMDATA + 1),
...
static void aw8693x_offset_cali(struct aw_haptic *aw_haptic)
{
	int os_code = 0;
	int d2s_gain = 0;
	uint8_t reg_val[3] = { 0 };
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_DETCFG1, AW8693X_BIT_DETCFG1_DET_GO_MASK,
				 AW8693X_BIT_DETCFG1_DET_GO_OFF);
	i2c_r_bytes(aw_haptic, AW8693X_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
	aw8693x_ram_init(aw_haptic, false);
				    ...
				    struct aw_haptic_container *awinic_cont)
{
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_RTPCFG3, reg_array, AW_I2C_BYTE_THREE);

	/* get FIFO_AE and FIFO_AF addr */
	i2c_r_bytes(aw_haptic, AW8693X_REG_RTPCFG3, reg_array, AW_I2C_BYTE_THREE);
	...
	while (i < awinic_cont->len) {
		...
		i2c_w_bytes(aw_haptic, AW8693X_REG_RAMDATA, &awinic_cont->data[i], len);
		i += len;
	}
	...
	while (i < awinic_cont->len) {
		...
		i2c_r_bytes(aw_haptic, AW8693X_REG_RAMDATA, ram_data, len);
...
static uint64_t aw8693x_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8693X_REG_SYSCTRL4, &reg_val, AW_I2C_BYTE_ONE);
			     ...
			     struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8693x_bst_vol_default",
...
static void aw8693x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bytes(aw_haptic, AW8693X_REG_SYSCTRL5, reg_val, AW_I2C_BYTE_ONE);
	/* Cont drv2 lvl */
	...
	/* Get gain */
	i2c_r_bytes(aw_haptic, AW8693X_REG_PLAYCFG2, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	/* Get wave_seq */
	i2c_r_bytes(aw_haptic, AW8693X_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	...
	i2c_w_bits(aw_haptic, AW8693X_REG_CONTCFG5,
				 AW8693X_BIT_CONTCFG5_BRK_GAIN_MASK, aw_haptic->info.cont_brk_gain);
	i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG10,
	...
	reg_val[5] = AW8693X_BIT_ANACFG3_INIT_VAL;
	aw8693x_reg_unlock(aw_haptic, true);
	/* uvlo adj default 2.4V */
	i2c_w_bits(aw_haptic, AW8693X_REG_SYSCTRL6,
				 AW8693X_BIT_SYSCTRL6_UVLO_ADJ_MASK,
				 aw_haptic->info.uvlo_vol_default);
	i2c_w_bytes(aw_haptic, AW8693X_REG_ANACFG7, reg_val, AW_I2C_BYTE_TWO);
	i2c_w_bytes(aw_haptic, AW8693X_REG_ANACFG10, &reg_val[2], AW_I2C_BYTE_ONE);
	i2c_w_bytes(aw_haptic, AW8693X_REG_ANACFG15, &reg_val[3], AW_I2C_BYTE_ONE);
	i2c_w_bytes(aw_haptic, AW8693X_REG_ANACFG21, &reg_val[4], AW_I2C_BYTE_ONE);
	i2c_w_bytes(aw_haptic, AW8693X_REG_ANACFG3, &reg_val[5], AW_I2C_BYTE_ONE);
 ...
 *
 ******************************************************/
/* Started by AICoder, pid:x5929k4e72f7a5c1472f09e401cb49295e38f8fb */
#ifdef DUAL_RTP_TEST
	...
	if (flag) {
		if (right->address_mode == AW8693X_NORMAL) {
			...
			i2c_w_bits(right, AW8693X_REG_SYSCTRL5,
								...
								AW8693X_BROADCAST_ADDR);
			reg = 0x68;
			i2c_w_bytes(right, AW8693X_REG_SYSCTRL2, &reg, AW_I2C_BYTE_ONE);
...
static ssize_t rtp_dual_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	i2c_r_bytes(left, AW8693X_REG_RTPCFG1, &reg_val[0], 1);
	i2c_r_bytes(left, AW8693X_REG_SYSCTRL2, &reg_val[1], 4);
	...
	right->i2c->addr = right->record_i2c_addr;
	i2c_r_bytes(right, AW8693X_REG_RTPCFG1, &reg_val[0], 1);
	i2c_r_bytes(right, AW8693X_REG_SYSCTRL2, &reg_val[1], 4);
...
/* Ended by AICoder, pid:x5929k4e72f7a5c1472f09e401cb49295e38f8fb */
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		mutex_lock(&aw_haptic->lock);
		i2c_r_bytes(aw_haptic, AW8693X_REG_CONTCFG6, reg_array, AW_I2C_BYTE_ONE);
		...
		i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG6, reg_array, AW_I2C_BYTE_TWO);
		mutex_unlock(&aw_haptic->lock);
	}
				   ...
				   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		mutex_lock(&aw_haptic->lock);
		i2c_w_bytes(aw_haptic, AW8693X_REG_CONTCFG8, reg_array, AW_I2C_BYTE_TWO);
		mutex_unlock(&aw_haptic->lock);
	}
...
struct aw_haptic_func aw8693x_func_list = {
	...
	.juge_rtp_going = aw8693x_judge_rtp_going,

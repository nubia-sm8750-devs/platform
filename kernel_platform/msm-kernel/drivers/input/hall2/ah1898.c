	...
	if (ah1898_chip_data->ah1898_enabled == 1) {
		...
		if (value == 1) {
			...
			*/
			...
			input_report_switch(ah1898_chip_data->input, SW_LID, 0);
			input_sync(ah1898_chip_data->input);
#endif
			hall_status = 1;
		} else {
...
#ifdef ZTE_FEATURE_SECONDARY_ENABLE
			input_report_switch(ah1898_chip_data->input, SW_LID, 1);
			input_sync(ah1898_chip_data->input);
#endif
			hall_status = 0;
		}
...
static irqreturn_t ah1898_interrupt(int irq, void *dev_id)
{
	...
	pr_info("%s:ah1898_2_interrupt!!\n", __func__);
	/*disable_irq_nosync(irq);*/
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 14, 0))
	__pm_wakeup_event(ah1898_chip_data->hall_wakelock, WAKELOCK_HOLD_TIME);
#else
	wake_lock_timeout(&ah1898_chip_data->hall_wakelock, msecs_to_jiffies(WAKELOCK_HOLD_TIME));
#endif
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 14, 0))
	ah1898_chip_data->hall_wakelock = wakeup_source_register(NULL, "hall_wakelock");
#else
	wake_lock_init(&ah1898_chip_data->hall_wakelock, WAKE_LOCK_SUSPEND, "hall_wakelock");
#endif
...
fail2:
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 14, 0))
	wakeup_source_unregister(ah1898_chip_data->hall_wakelock);
#else
	wake_lock_destroy(&ah1898_chip_data->hall_wakelock);
#endif
fail1:

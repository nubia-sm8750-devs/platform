...
static int silfp_irq_to_reset_init(struct silfp_data *fp_dev)
{
    ...
    if (fp_dev->int_port >= 0) {
        ...
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d",__func__,(int32_t)fp_dev->int_port, ret);
            return -ENODEV;
        } else {
            gpio_direction_output(fp_dev->int_port, 0);
        }
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    if (fp_dev->avdd_port >= 0 ) {
        ...
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d",__func__,(int32_t)fp_dev->avdd_port, ret);
            status = -ENODEV;
            goto err_avdd;
        }
    }
    ...
    if (fp_dev->vddio_port >= 0 ) {
        ...
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d",__func__,(int32_t)fp_dev->vddio_port, ret);
            status = -ENODEV;
            goto err_vddio;
        }
    }
    ...
    if (fp_dev->rst_port >= 0 ) {
        ...
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d",__func__,(int32_t)fp_dev->rst_port, ret);
            status = -ENODEV;
            goto err_rst;
        } else {
            gpio_direction_output(fp_dev->rst_port, 1);

...
/* Started by AICoder, pid:601295df41ga02614fe40bba1027ad0fb795c9b9 */
#ifdef CONFIG_DRM_ZTE_DISP_LTM
    // This macro defines the maximum level for ZTE BF237 in normal mode.
    // The value 3516 represents the upper limit for the specified configuration.
    #define ZTE_BF237_NORMAL_MAX_LEVEL 3516
#endif
/* Ended by AICoder, pid:601295df41ga02614fe40bba1027ad0fb795c9b9 */
...
/* Started by AICoder, pid:n89439eb12yeda914a6c0a4d8048341de2e948b0 */
#ifdef CONFIG_DRM_ZTE_DISP_LTM
    /* Scale the backlight level */
    bl_temp = bl_config->bl_level * bl_config->bl_scale / MAX_BL_SCALE_LEVEL;
    bl_ltm_temp = (u32)bl_temp * bl_config->bl_scale_sv / MAX_SV_BL_SCALE_LEVEL;

    /* Check if the scaled value exceeds the maximum allowed levels and adjust accordingly */
    if (bl_ltm_temp > ZTE_BF237_NORMAL_MAX_LEVEL && bl_temp <= ZTE_BF237_NORMAL_MAX_LEVEL && \
				...
				struct drm_connector *connector)
{
	...
	#ifdef CONFIG_DRM_ZTE_DISP
	/*we should check ONNECTOR_PROP_ZTE_HDR_RATIO first*/
	u64 isdim = sde_connector_get_property(connector->state, CONNECTOR_PROP_ZTE_HDR_RATIO);
    #endif
		...
		uint64_t val)
{
	...
	case CONNECTOR_PROP_LP:
		...
		break;
	#ifdef CONFIG_DRM_ZTE_DISP_FOD
	case CONNECTOR_PROP_ZTE_LAYER:
		/* suspend case: clear stale MISR */
		zte_spec_layer_report(c_conn, c_state, val);
	...
	struct msm_display_info *display_info)
{
	...
	if (connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		#ifdef CONFIG_DRM_ZTE_DISP_FOD
		msm_property_install_range(&c_conn->property_info, "zte_spec_layer",
					0x0, 0, ~0, 0, CONNECTOR_PROP_ZTE_LAYER);
		#endif
		#ifdef CONFIG_DRM_ZTE_DISP
		msm_property_install_range(&c_conn->property_info, "zte_hdr_ratio",
					0x0, 0, ~0, 0, CONNECTOR_PROP_ZTE_HDR_RATIO);
		#endif

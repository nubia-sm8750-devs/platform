...
/* Started by AICoder, pid:nf74cn0e8ebc564148e508dd0092cd045d21486a */
	{ FILTERED_MODE(REDMAGIC_GM001J, UGREEN_EXT_CM179_80133, 1920, 1080, 120, 0) },
/* Ended by AICoder, pid:nf74cn0e8ebc564148e508dd0092cd045d21486a */
	{ FILTERED_MODE(SKYWORTH_F24G40F, BASEUS_EXT_BS_OH160, 1920, 1080, 120, 0) },
	{ FILTERED_MODE(SKYWORTH_F24G40F, BASEUS_EXT_BS_OH160, 1920, 1080, 100, 0) },
	{ FILTERED_MODE(KTC_KTCM27P20, UGREEN_EXT_CM498_15601, 1920, 1080, 120, 0) },
	{ FILTERED_MODE(REDMAGIC_GM001S, UGREEN_EXT_CM179_80133, 1920, 1080, 120, 0) },
...
//+linx for dp eye diagram test
#define EYE_DIAGRAM_DATA_COUNT (16)
#define DP_EYE_DIAGRAM_PARAM_IS_NULL ("dp eye diagram param is NULL\n")
static u8* swing_hbr2_3 = NULL;
static u8* pre_emp_hbr2_3 = NULL;
static u8* swing_hbr_rbr = NULL;
static u8* pre_emp_hbr_rbr = NULL;

void setDPEyeDiagramParam(struct dp_catalog_ctrl *ctrl){
	if(ctrl){
		DP_INFO("%s()", __func__);
		swing_hbr2_3 = ctrl->swing_hbr2_3;
		pre_emp_hbr2_3 = ctrl->pre_emp_hbr2_3;
		swing_hbr_rbr = ctrl->swing_hbr_rbr;
		pre_emp_hbr_rbr = ctrl->pre_emp_hbr_rbr;
	}
}

static ssize_t swing_hbr2_3_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	u16 len = 0;
	if(!swing_hbr2_3 || !pre_emp_hbr2_3 || !swing_hbr_rbr || !pre_emp_hbr_rbr){
		DP_INFO("%s", DP_EYE_DIAGRAM_PARAM_IS_NULL);
		len = snprintf(buf, SZ_32, DP_EYE_DIAGRAM_PARAM_IS_NULL);
	}else{
		int i = 0;
		char tmpBuff[20] = {0};
		buf[0] = 0;
		for(i = 0; i < EYE_DIAGRAM_DATA_COUNT; ++i){
			sprintf(tmpBuff, "%02X ", swing_hbr2_3[i]);
			strcat(buf, tmpBuff);
		}
		strcat(buf, "\n");
		len = strlen(buf);
...
static ssize_t swing_hbr2_3_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	int i = 0;
	int array[EYE_DIAGRAM_DATA_COUNT] = {0};

	if (EYE_DIAGRAM_DATA_COUNT != sscanf(buf, "%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x"
			, &array[0]  , &array[1]  , &array[2]  , &array[3], &array[4]  , &array[5]  , &array[6]  , &array[7]
			, &array[8]  , &array[9]  , &array[10] , &array[11], &array[12] , &array[13] , &array[14] , &array[15])) 
	{
		DP_INFO("set param error\n");
	}else{
		DP_INFO("set param ok\n");
		for(i = 0; i < EYE_DIAGRAM_DATA_COUNT; ++i){
			swing_hbr2_3[i] = (u8)array[i];
		}
	}
	return size;
}

static ssize_t pre_emp_hbr2_3_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	u16 len = 0;
	if(!swing_hbr2_3 || !pre_emp_hbr2_3 || !swing_hbr_rbr || !pre_emp_hbr_rbr){
		DP_INFO("%s", DP_EYE_DIAGRAM_PARAM_IS_NULL);
		...
		buf[0] = 0;
		for(i = 0; i < EYE_DIAGRAM_DATA_COUNT; ++i){
			sprintf(tmpBuff, "%02X ", pre_emp_hbr2_3[i]);
			strcat(buf, tmpBuff);
		}
		strcat(buf, "\n");
		len = strlen(buf);
...
static ssize_t pre_emp_hbr2_3_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	int i = 0;
	int array[EYE_DIAGRAM_DATA_COUNT] = {0};
			...
			, &array[8]  , &array[9]  , &array[10] , &array[11], &array[12] , &array[13] , &array[14] , &array[15])) 
	{
		...
		DP_INFO("set param ok\n");
		memcpy(pre_emp_hbr2_3, array, sizeof(array));
		for(i = 0; i < EYE_DIAGRAM_DATA_COUNT; ++i){
			pre_emp_hbr2_3[i] = (u8)array[i];
		}
	}
	return size;
}

static ssize_t swing_hbr_rbr_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	u16 len = 0;
	if(!swing_hbr2_3 || !pre_emp_hbr2_3 || !swing_hbr_rbr || !pre_emp_hbr_rbr){
		DP_INFO("%s", DP_EYE_DIAGRAM_PARAM_IS_NULL);
		...
		buf[0] = 0;
		for(i = 0; i < EYE_DIAGRAM_DATA_COUNT; ++i){
			sprintf(tmpBuff, "%02X ", swing_hbr_rbr[i]);
			strcat(buf, tmpBuff);
		}
		strcat(buf, "\n");
		len = strlen(buf);
...
static ssize_t swing_hbr_rbr_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	int i = 0;
	int array[EYE_DIAGRAM_DATA_COUNT] = {0};
			...
			, &array[8]  , &array[9]  , &array[10] , &array[11], &array[12] , &array[13] , &array[14] , &array[15])) 
	{
		...
		DP_INFO("set param ok\n");
		memcpy(swing_hbr_rbr, array, sizeof(array));
		for(i = 0; i < EYE_DIAGRAM_DATA_COUNT; ++i){
			swing_hbr_rbr[i] = (u8)array[i];
		}
	}
	return size;
}

static ssize_t pre_emp_hbr_rbr_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	u16 len = 0;
	if(!swing_hbr2_3 || !pre_emp_hbr2_3 || !swing_hbr_rbr || !pre_emp_hbr_rbr){
		DP_INFO("%s", DP_EYE_DIAGRAM_PARAM_IS_NULL);
		...
		buf[0] = 0;
		for(i = 0; i < EYE_DIAGRAM_DATA_COUNT; ++i){
			sprintf(tmpBuff, "%02X ", pre_emp_hbr_rbr[i]);
			strcat(buf, tmpBuff);
		}
		strcat(buf, "\n");
		len = strlen(buf);
...
static ssize_t pre_emp_hbr_rbr_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t size)
{
	int i = 0;
	int array[EYE_DIAGRAM_DATA_COUNT] = {0};
...
static struct kobj_attribute disp_attrs[] = {
	...
	//+linx for dp eye diagram test
	__ATTR(swing_hbr2_3, 0664, swing_hbr2_3_show, swing_hbr2_3_store),
	__ATTR(pre_emp_hbr2_3, 0664, pre_emp_hbr2_3_show, pre_emp_hbr2_3_store),
	__ATTR(swing_hbr_rbr, 0664, swing_hbr_rbr_show, swing_hbr_rbr_store),
	__ATTR(pre_emp_hbr_rbr, 0664, pre_emp_hbr_rbr_show, pre_emp_hbr_rbr_store),
	//-linx
};

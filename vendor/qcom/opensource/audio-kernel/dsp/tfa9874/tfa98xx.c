	...
	size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		...
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_info("[0x%x] tfa_dev_start complete: %d\n", tfa98xx->i2c->addr, ret);
	}
		...
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_info("[0x%x] tfa_dev_stop complete: %d\n", tfa98xx->i2c->addr, ret);
	}
	...
	loff_t *ppos)
{
	...
	if (tfa98xx->tfa == NULL) {
		pr_info("[0x%x] dsp is not available\n", tfa98xx->i2c->addr);
		return -ENODEV;
	}
	...
	if (buffer == NULL) {
		pr_info("[0x%x] can not allocate memory\n", tfa98xx->i2c->addr);
		return -ENOMEM;
	}
...
//		kfree(msg_file);
	...
	if (tfa98xx->tfa->is_probus_device) {
    	...
    	if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
    		...
    		if (error != Tfa98xx_Error_Ok) {
    			pr_info("[0x%x] tfaContWriteFile error: %d\n", tfa98xx->i2c->addr, error);
    			err = -EIO;
    		}
    	}
    	else {
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		pr_info("Skipping tfa_dev_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		return;
	}
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	pr_info("enter tfa98xx_probe QCh \n");

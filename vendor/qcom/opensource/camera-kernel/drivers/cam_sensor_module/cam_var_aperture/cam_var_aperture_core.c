...
#define  CAM_ACTUATOR_PACKET_OPCODE_SKIP     4
#define  CAM_ACTUATOR_PACKET_OPCODE_SKIP0    5
#define  CAM_ACTUATOR_PACKET_OPCODE_SKIP1    6
#define  CAM_ACTUATOR_PACKET_OPCODE_SKIP2    7
#define  CAM_ACTUATOR_PACKET_OPCODE_SKIP3    8


unsigned long long aperture_skip_count = 0 ;
static int countinuous_nop_frame = 0;
static int aperture_move_flag = 0;
uint32_t gpio_avdd = 0;

module_param(aperture_skip_count, ullong, 0644);

ssize_t camera_open_zte_var_aperture(struct cam_var_aperture_ctrl_t *pa_ctrl){
    int ret = 0;
...
ssize_t camera_close_zte_var_aperture(struct cam_var_aperture_ctrl_t *pa_ctrl){
...
ssize_t camera_zte_adj_aperture_switch_state(unsigned int step_value, struct cam_var_aperture_ctrl_t *pa_ctrl){
    int ret = 0;
...
ssize_t camera_zte_adj_aperture_stream_on(struct cam_var_aperture_ctrl_t *pa_ctrl){
    int ret = 0;
...
static ssize_t camera_zte_adj_aperture_stream_off(struct cam_var_aperture_ctrl_t *pa_ctrl){
    int ret = 0;
...
int32_t cam_var_aperture_construct_default_power_setting(
...
static int32_t cam_var_aperture_power_up(struct cam_var_aperture_ctrl_t *a_ctrl)
{
    int rc = 0;
    struct cam_hw_soc_info                 *soc_info = &a_ctrl->soc_info;
    struct cam_var_aperture_soc_private        *soc_private;
    struct cam_sensor_power_ctrl_t         *power_info;
    struct completion                      *i3c_probe_completion = NULL;

    return  0;
    soc_private =
        (struct cam_var_aperture_soc_private *)a_ctrl->soc_info.soc_private;
    ...
    if ((power_info->power_setting == NULL) &&
        ...
        rc = cam_var_aperture_construct_default_power_setting(power_info);
        if (rc < 0) {
    ...
    if (a_ctrl->io_master_info.master_type == I3C_MASTER)
        i3c_probe_completion = cam_var_aperture_get_i3c_completion(a_ctrl->soc_info.index);
...
static int32_t cam_var_aperture_power_down(struct cam_var_aperture_ctrl_t *a_ctrl)
{
    int32_t rc = 0;
    struct cam_sensor_power_ctrl_t *power_info;
    struct cam_hw_soc_info *soc_info = &a_ctrl->soc_info;
...
static int32_t cam_var_aperture_i2c_modes_util(
...
int32_t cam_var_aperture_slaveInfo_pkt_parser(struct cam_var_aperture_ctrl_t *a_ctrl,
    uint32_t *cmd_buf, size_t len)
{
    ...
    if (a_ctrl->io_master_info.master_type == CCI_MASTER) {
        ...
        a_ctrl->io_master_info.qup_client->i2c_client->addr = 0x6c;//i2c_info->slave_addr;//zteadd
...
int32_t cam_var_aperture_apply_settings(struct cam_var_aperture_ctrl_t *a_ctrl,
    struct i2c_settings_array *i2c_set)
{
    ...
    list_for_each_entry(i2c_list,
        ...
        rc = cam_var_aperture_i2c_modes_util(
...
int32_t cam_var_aperture_apply_request(struct cam_req_mgr_apply_request *apply)
{
    ...
    a_ctrl = (struct cam_var_aperture_ctrl_t *)
    ...
    if ((apply->request_id ==
        ...
        == 1) {
        rc = cam_var_aperture_apply_settings(a_ctrl,
...
int32_t cam_var_aperture_establish_link(
...
static int cam_var_aperture_update_req_mgr(
    ...
    struct cam_packet *csl_packet)
{
    ...
    skipNum = csl_packet->header.op_code & 0xFFFFF;

    if ((skipNum >= CAM_ACTUATOR_PACKET_OPCODE_SKIP0) && (skipNum <= CAM_ACTUATOR_PACKET_OPCODE_SKIP3)) {
            add_req.trigger_eof = true;
            add_req.skip_at_eof = 0;
            add_req.skip_at_sof = skipNum - CAM_ACTUATOR_PACKET_OPCODE_SKIP0 ;
            if(aperture_skip_count)
            {
                add_req.skip_at_sof = aperture_skip_count;
            }
            camera_zte_adj_aperture_stream_on(a_ctrl);
            countinuous_nop_frame = 0;
            aperture_move_flag = 1;
            CAM_DBG(CAM_ACTUATOR, "skip_at_sof  %d csl_packet->header.request_id %d", add_req.skip_at_sof, csl_packet->header.request_id);
    }

    if((CAM_ACTUATOR_PACKET_NOP_OPCODE == skipNum) || (skipNum == CAM_ACTUATOR_PACKET_OPCODE_SKIP))
    {
        if(aperture_move_flag)
        {
            countinuous_nop_frame++;
            if(countinuous_nop_frame >= 15)
            {
                aperture_move_flag = 0;
                countinuous_nop_frame = 0;
...
int32_t cam_var_aperture_publish_dev_info(struct cam_req_mgr_device_info *info)
{
    struct cam_var_aperture_ctrl_t *a_ctrl;
    ...
    snprintf(info->name, sizeof(info->name), "%s(camera-aperture%u)",
        CAM_APERTURE_NAME, a_ctrl->soc_info.index);
...
#if  0
const  uint32_t cur_new_fw_ver = 24101600;
bool cam_aperture_need_update_fw(struct cam_var_aperture_ctrl_t *a_ctrl)
{
    uint32_t read_fw_ver = 0;
    int ret = 0;

    ret = camera_io_dev_read(&a_ctrl->io_master_info, 0x00FC, &read_fw_ver, CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_DWORD, 0);
    if(!ret)
    {
        CAM_INFO(CAM_ACTUATOR, "fwdata  read_fw_ver: %0x", read_fw_ver);
        if((cur_new_fw_ver > read_fw_ver) && (read_fw_ver >= 24082504))
        {
            CAM_INFO(CAM_ACTUATOR, "fwdata  read_fw_ver: %0x", read_fw_ver);
            return 1;
        }
...
int32_t cam_var_aperture_i2c_pkt_parse(struct cam_var_aperture_ctrl_t *a_ctrl,
    void *arg)
{
    int32_t  rc = 0;
    int32_t  i = 0;
    ...
    case CAM_ACTUATOR_PACKET_OPCODE_INIT:
        ...
        for (i = 0; i < csl_packet->num_cmd_buf; i++) {
            ...
            case CAMERA_SENSOR_CMD_TYPE_I2C_INFO:
                ...
                rc = cam_var_aperture_slaveInfo_pkt_parser(
        ...
        if (a_ctrl->cam_act_state == cam_var_aperture_ACQUIRE) {
            rc = cam_var_aperture_power_up(a_ctrl);
            camera_open_zte_var_aperture(a_ctrl);
            camera_zte_adj_aperture_stream_on(a_ctrl);
            camera_zte_adj_aperture_switch_state(0xfff, a_ctrl);
            //cam_aperture_need_update_fw(a_ctrl);
            if (rc < 0) {
                CAM_ERR(CAM_ACTUATOR,
                    "aperture Power up failed");
                goto end;
            }
            a_ctrl->cam_act_state = cam_var_aperture_CONFIG;
        }
#if  0
        rc = cam_var_aperture_apply_settings(a_ctrl,
        ...
        //camera_zte_adj_aperture_switch_state(0, a_ctrl);
    ...
    case CAM_ACTUATOR_PACKET_AUTO_MOVE_LENS:
        if (a_ctrl->cam_act_state < cam_var_aperture_CONFIG) {
            rc = -EINVAL;
            CAM_WARN(CAM_ACTUATOR,
        ...
        a_ctrl->setting_apply_state = ACT_APPLY_SETTINGS_LATER_APERTURE;
        i2c_data = &(a_ctrl->i2c_data);
        ...
        rc = cam_var_aperture_update_req_mgr(a_ctrl, csl_packet);
        if (rc) {
            CAM_ERR(CAM_ACTUATOR,
    ...
    case CAM_ACTUATOR_PACKET_OPCODE_SKIP:
        if (a_ctrl->cam_act_state < cam_var_aperture_CONFIG) {
            CAM_WARN(CAM_ACTUATOR,
                "Received NOP packets in invalid state: %d",
                a_ctrl->cam_act_state);
        ...
        break;
    case CAM_ACTUATOR_PACKET_OPCODE_SKIP0:
    case CAM_ACTUATOR_PACKET_OPCODE_SKIP1:
    case CAM_ACTUATOR_PACKET_OPCODE_SKIP2:
    case CAM_ACTUATOR_PACKET_OPCODE_SKIP3:
        if (a_ctrl->cam_act_state < cam_var_aperture_CONFIG) {
            rc = -EINVAL;
            CAM_WARN(CAM_ACTUATOR,
                "Not in right state to move lens: %d",
...
void cam_var_aperture_move_max_adc(struct cam_var_aperture_ctrl_t *a_ctrl)
{
    if(a_ctrl->cam_act_state  != cam_var_aperture_INIT)
    {
         camera_io_init(&a_ctrl->io_master_info);
         camera_zte_adj_aperture_stream_on(a_ctrl);
         camera_zte_adj_aperture_switch_state(0xfff, a_ctrl);
         camera_io_release(&a_ctrl->io_master_info);
    }
...
void cam_var_aperture_shutdown(struct cam_var_aperture_ctrl_t *a_ctrl)
{
    int rc = 0;
    struct cam_var_aperture_soc_private  *soc_private =
        (struct cam_var_aperture_soc_private *)a_ctrl->soc_info.soc_private;
    ...
    if (a_ctrl->cam_act_state >= cam_var_aperture_CONFIG) {
        rc = cam_var_aperture_power_down(a_ctrl);
        camera_close_zte_var_aperture(a_ctrl);
        if (rc < 0)
            CAM_ERR(CAM_ACTUATOR, "Actuator Power down failed");
        a_ctrl->cam_act_state = cam_var_aperture_ACQUIRE;
    }
...
int32_t cam_var_aperture_driver_cmd(struct cam_var_aperture_ctrl_t *a_ctrl,
    void *arg)
{
    int rc = 0;
    ...
    case CAM_RELEASE_DEV: {
        if (a_ctrl->cam_act_state == cam_var_aperture_START) {
            rc = -EINVAL;
            CAM_WARN(CAM_ACTUATOR,
    ...
    case CAM_START_DEV: {
        ...
        a_ctrl->cam_act_state = cam_var_aperture_START;
        a_ctrl->last_flush_req = 0;
    }
        break;
    case CAM_STOP_DEV: {
    ...
    case CAM_CONFIG_DEV: {
        ...
        rc = cam_var_aperture_i2c_pkt_parse(a_ctrl, arg);
        if (rc < 0) {
            if (rc == -EBADR)
                CAM_INFO(CAM_ACTUATOR,
        ...
        if (a_ctrl->setting_apply_state ==
            ...
            CAM_ERR(CAM_ACTUATOR, "crmerr  Invalid Opcod");
            rc = cam_var_aperture_apply_settings(a_ctrl,
                &a_ctrl->i2c_data.init_settings);
            if ((rc == -EAGAIN) &&
...
int32_t cam_var_aperture_flush_request(struct cam_req_mgr_flush_request *flush_req)
{
    int32_t rc = 0, i;

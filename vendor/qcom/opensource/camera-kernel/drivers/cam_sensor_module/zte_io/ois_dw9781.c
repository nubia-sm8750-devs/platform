...
int ois_dw9781c_debugfs_cal_s(void *data)
{
    ...
    RamRead16A_ois(data, 0x70F0, &xRaw);  /* x gyro raw */
    RamRead16A_ois(data, 0x70F1, &yRaw);  /* y gyro raw */
    RamRead16A_ois(data, 0x7018, &interceptMode);  /* master or intercept mode */
    
	pr_info("%s:%d:[dw9781c_gyro_offset_calibrtion] msg : %d", __func__, __LINE__, msg);
    pr_info("%s:%d:[dw9781c_gyro_offset_calibrtion] x_gyro_offset 0x%04X, y_gyro_offset 0x%04X, xRaw 0x%04X, yRaw 0x%04X interceptMode %d",
